...
status: {status}            # draft | approved | implemented | verified | signed-off | machine-cleared | da-cham-boi-thuc-te — LUÔN `draft` lúc tạo; Cổng 1 → approved; vòng làm → implemented/verified; Cổng 2 → signed-off (người ký) hoặc machine-cleared (máy đã thông, xanh-sạch, không chữ ký)
approved_by:
approved_at:
---
```
