...
1. **Chân sản phẩm** — giá trị cụ thể của sản phẩm này (kênh, surface, role, thị trường, domain map…). Thang nguồn, lấy nấc cao nhất có: (a) mục `## Product Context` trong CLAUDE.md hoặc `docs/product-context.md` → (b) chưa có mục đó nhưng repo có đồ: đào glossary/tài liệu đặc tả/schema/code rồi TỰ DỰNG bản Product Context — mỗi dòng gắn `[SUY-TỪ-REPO: <đường dẫn>]` (nguồn có thật vừa đọc; cấm bịa đường dẫn) hoặc `[GIẢ ĐỊNH]`, KHÔNG dừng phỏng vấn → (c) repo mới tinh: vẫn TỰ DỰNG theo đúng khuôn 5 ý (sản phẩm gì; loại hình — marketplace mấy phía / SaaS / tool nội bộ; actor & phía; surface + kênh; thị trường & khung pháp lý) bằng `[GIẢ ĐỊNH]` và `[NGÀNH]` từ chân ngành. Mọi dòng `[GIẢ ĐỊNH]`/`[NGÀNH]` gom về mục Coverage của contract để người gạch MỘT lượt tại Cổng 1 — cùng số quyết định, một lần ngồi, và Cổng 1 vốn đã cần người nên không thêm lượt gọi nào (hồ sơ doi-hanh-vi-cong-nguoi, owner gạch 12/08). Sau (b)/(c) đề nghị lưu vào CLAUDE.md theo mẫu `references/product-context-template.md` để lần sau khỏi dựng lại.
...
Nhãn nguồn cho giá trị không hiển nhiên: `[SUY-TỪ-REPO: <đường dẫn>]` = truy được chân sản phẩm từ vật có thật trong repo (đường dẫn phải tồn tại; nấc (a) Product Context đã duyệt cũng dùng nhãn này, trỏ file chứa mục đó) · `[NGÀNH: <tên>]` = từ chân ngành, vào scan làm ỨNG VIÊN chờ user gật/cắt chứ không phải fact · `[GIẢ ĐỊNH]` = không truy được đâu — gom về Coverage chờ người gạch một lượt tại Cổng 1 trước khi tuyên "đủ". "Chuẩn ngành" mà không nêu được tên = `[GIẢ ĐỊNH]`. Ví dụ gắn nhãn "vd" trong preset chỉ minh họa hình dạng — cấm dùng làm giá trị mặc định cho sản phẩm khác.
...
- Giá trị không gắn được nhãn nguồn nào (`[SUY-TỪ-REPO: đường dẫn]` / `[NGÀNH: tên]` / `[GIẢ ĐỊNH]`) → đang bịa theo ngữ cảnh của sản phẩm khác.
- Mọi giá trị đều `[SUY-TỪ-REPO:]` → scan chỉ chép lại điều repo đã biết, không chặn được sót — nguồn được chép chính là nguồn đang sót. Repo mới tinh mà thiếu chân ngành là dạng nặng nhất của lỗi này.
