...
1. **Chân sản phẩm** — giá trị cụ thể của sản phẩm này (kênh, surface, role, thị trường, domain map…). Thang nguồn, lấy nấc cao nhất có: (a) mục `## Product Context` trong CLAUDE.md hoặc `docs/product-context.md` → (b) chưa có mục đó nhưng repo có đồ: đào glossary/spec/schema/code rồi **TỰ DỰNG** Product Context → (c) repo mới tinh: **TỰ DỰNG** từ những gì đọc được (README, package.json, schema, tên route/model), phần nào không suy được thì để nguyên là giả định.

   **(b) và (c) KHÔNG hỏi trước từng cái.** Máy dựng trọn bản Product Context, và **mỗi dòng gắn nhãn nguồn**: `[SUY-TỪ-REPO: <đường dẫn>]` khi truy được về một tệp thật trong repo, `[GIẢ ĐỊNH]` khi không. Mọi dòng `[GIẢ ĐỊNH]` và `[NGÀNH]` **gom vào mục Coverage của contract** để người gạch **một lượt tại Cổng 1** — chứ không chặn người 5 lần lúc đang quét.

   Vì sao đổi: hỏi trước từng ý là bắt người trả lời những thứ máy đọc được từ chính repo đang mở, và trả lời lúc chưa thấy bảng thì người cũng chưa biết ý nào quan trọng. Gom vào Cổng 1 thì người đọc CẢ bảng rồi gạch — cùng số quyết định, một lần ngồi, và Cổng 1 vốn đã cần người nên **không thêm lượt gọi nào**. Nhãn nguồn là thứ giữ cho việc này không thành bịa: một dòng `[GIẢ ĐỊNH]` chưa ai gạch thì **cấm đếm vào "đã đủ"**.

   Sau (b)/(c) đề nghị lưu vào CLAUDE.md theo mẫu `references/product-context-template.md` để lần sau khỏi dựng lại.
