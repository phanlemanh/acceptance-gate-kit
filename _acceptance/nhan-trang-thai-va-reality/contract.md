...
- AC-10: Given hồ sơ `status: da-cham-boi-thuc-te`, When chạy `pre-merge-check.sh` VÀ `recheck-evidence.cjs --recheck-all`, Then ở CẢ HAI bên đọc: dòng `thuc-te` hợp lệ → NOTE «đã chấm bởi thực tế — chạy trên prod từ bản dựng <sha7> (quan sát <ngày>, <tên>)» và không VIOLATION nào từ luật verdict/chữ ký; dòng thiếu một vế → VIOLATION gọi tên vế; `build_sha` không tồn tại trong kho (`git cat-file -e` trượt) → VIOLATION ghim «bản dựng không có trong kho»; đổi `evals.yaml` hoặc `rang/**` của hồ sơ ấy sau dòng quan sát → VIOLATION «khoá việc thước»; dòng `supersedes` trỏ id dòng `thuc-te` → hồ sơ về luật cũ; hai bên gọi CÙNG vị từ `thucTe` của lib. (cross-layer)
