...
surfaces: [cli, ci]
status: machine-cleared      # draft | approved | implemented | verified | signed-off | machine-cleared
approved_by: Mạnh
...
- Giới hạn đã khai (lượt 4, 23/09), kèm ngưỡng: L3 của bên đọc không neo dòng, nên năm hình dạng override mà L3 đếm chốt KHÔNG chạm — giữa dòng bảng, trong chú thích, khoá có tiền tố trùng đuôi, bản ghi một dòng kiểu `{…}`, nội dung khối vô hướng. Danh sách một nguồn là khối `GIOI-HAN-CHOT` trong tệp workflow; ca vi phân đòi mỗi tên có ô và mỗi ô có tên. Nghiệm đúng tầng là neo L3 (hạt giống vế 2). Ngưỡng mở lại: ≥ 1 báo cáo thật mang override ở một trong năm hình dạng.
...
- **Known limits (owner quyết ở Cổng Bằng chứng 23/09):**
  - ~~Ngoài-1~~ — owner nâng phạm vi sau chữ ký (23/09): bản ghi carry lồng trong frontmatter giữ giờ carry, sửa ở S4 lượt 5 dưới AC-2 (sổ quyết định, dòng revisit gate2 thay d-…-15).
