...
# P197 / P199 (khong tin exit code tron suite), pha thu hai chieu rang cua
...
# (rang.sh cua hinh-tai-cong-1, moc diffBase 7d76384) do O DAY, khong o P199.
set -uo pipefail
...
O198="$(ONLY_BLOCK=P199 bash tests/plugins/run-tests.sh 2>&1)"; S198=$?
[ $S90 -eq 0 ] && [ $S197 -eq 0 ] && [ $S198 -eq 0 ] || keu "suite khoi P90/P197/P199 khong xanh ($S90/$S197/$S198)"
...
# AC-1 / AC-7 — P199
has "$O198" "PASS: P199" "khong thay PASS: P199"
for c in a-hai-ban-dung b-sua-giua c-sua-4-chu-dau d-sua-4-chu-cuoi e-xoa-mot-ban f-xoa-ca-hai; do has "$O198" "P199-CA-$c OK" "thieu ca fixture $c"; done
has "$O198" "P199 OK: 6 ca fixture · 4 kiem cau truc · 4 dot bien" "dong tong ket P199 lech"
...
# AC-7 — P199 tu canh minh khong doc thu muc ho so: chen mot dong co chuoi do vao ban sao khoi P199 → P199 DO
python3 - "$ROOT" "$CP" <<'PYX'
import sys
from pathlib import Path
src = (Path(sys.argv[1]) / "tests/plugins/run-tests.sh").read_text(encoding="utf-8")
i = src.index('run "P199 hfl_clause'); j = src.index("\nPY\n", i)
# P199 tu soi VAN BAN khoi cua no: chen mot dong mang chuoi thu muc ho so -> phai DO
Path(sys.argv[2]).write_text(src[:j] + "\n# thu: _acceptance/ban-sao\n" + src[j:], encoding="utf-8")
PYX
OUT7="$(ONLY_BLOCK=P199 bash "$CP" 2>&1)"; rm -f "$CP"
has "$OUT7" "P199 khong duoc doc thu muc ho so" "chen '_acceptance/' vao khoi P199 ma P199 khong do"

# AC-8 — tinh phan biet TREN DIFFBASE THAT cua nhanh nay (merge-base voi main = 7d76384, tuc
# ngay sau PR #62): o do P197 da co nhung CHUA in P197-M / P197-M-COUNT, va P199 chua ton tai —
# rang cua hinh-tai-cong-1 phai DO vi hop dong P197-M (vat cua ho so NAY), khong phai vi khoi
# GATE 1 vang; va suite base khong co khoi P199.
...
if git cat-file -e "$BASE^{commit}" 2>/dev/null; then
  if git worktree add -q "$TMP/base" "$BASE" 2>"$TMP/wt.err"; then
    ...
    OUT198B="$(ONLY_BLOCK=P199 bash "$TMP/base/tests/plugins/run-tests.sh" 2>&1)"
    has "$OUT198B" "ONLY_BLOCK=P199 khong khop khoi nao" "suite base khong bao 'khong khop khoi nao' cho P199 — P199 da ton tai o base?"
...
echo "SIET-RANG OK: P90 3 ban chep · P197 $NM msg + 5 tach + 5 nhan · P199 6 ca + tu-canh · rang cu 2 chieu do + phan biet diffBase"
exit 0
