...
- AC-7: Given `run-log.jsonl` của CHÍNH hồ sơ này SAU khi Workflow S4 đã append (tức lúc trình Cổng 2, không phải lúc chạy eval), When chạy `node tests/scripts/run-log-minted.mjs --slug <slug> --root .` (script S3 viết), Then mọi dòng của vòng chấm cuối có `run_id` khớp hình dạng Workflow đúc — `minted-<slug>-<id>-r<n>` với `<id>` ∈ tập id trong `evals.yaml` ∪ dạng `SUITE-<khoá executor>` — và cùng một `ts` mỗi vòng; script thoát 1 kèm thông điệp ghim khi run-log VẮNG («chua co run-log — AC-7 CHUA do»), khi có dòng lệch hình dạng, hoặc khi hai `ts` trong một vòng; đối chứng hai chiều trên fixture code-sinh (run-log tay → đỏ; run-log đúc → xanh). Đầu ra thật của script dán vào `evidence-report.md` mục Evidence; KHÔNG là eval (eval chạy trước khi run-log tồn tại → fail-open — gap-probe vòng 2).
