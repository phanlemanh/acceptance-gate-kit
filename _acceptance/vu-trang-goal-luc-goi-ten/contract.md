...
- AC-7: Given `run-log.jsonl` của CHÍNH hồ sơ này SAU khi main loop append `result.runLog` của Workflow S4 (tức lúc trình Cổng 2, không phải lúc chạy eval), When chạy `node tests/scripts/run-log-minted.mjs --slug <slug> --root . --usage`, Then vòng chấm cuối khớp HỢP ĐỒNG CỦA BÊN VIẾT `acceptance-verify.js` (S4-r1 sửa — bên viết có hai nhánh run_id: verifier khai, hoặc đúc `minted-…`; vòng chạy lại cùng round có nhiều `ts`): mọi dòng eval có `run_id` KHÁC RỖNG, mọi `evalId` (trừ `SUITE-…`) thuộc tập id đọc qua CHÍNH bộ đọc dùng chung `lib/eval-yaml.js` (không mọc parser thứ ba), cùng round cùng `sha`, và `usage-report.md` (wf-usage.mjs sinh từ transcript workflow) có mục «S4 round <k>» — đó là vết máy-giữ của việc đi qua Workflow; script thoát 1 kèm thông điệp ghim khi run-log vắng («chua co run-log — AC-7 CHUA do»), run_id rỗng, evalId ngoài evals, sha lệch, hay thiếu mục usage; đối chứng hai chiều trên fixture code-sinh. Đầu ra thật của script dán vào `evidence-report.md` mục Evidence; KHÔNG là eval (eval chạy trước khi run-log tồn tại → fail-open — gap-probe vòng 2). *Giới hạn khai thẳng: hình dạng chuỗi run_id KHÔNG chứng minh được nguồn gốc workflow (bên viết cho phép verifier khai run_id) — vết mạnh là usage-report từ transcript.*
