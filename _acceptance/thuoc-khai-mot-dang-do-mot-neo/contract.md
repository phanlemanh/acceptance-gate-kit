...
- AC-5: Given cây lành, When chạy `ONLY_BLOCK="P86 GATE-MODEL" bash tests/plugins/run-tests.sh` (selector phải là chuỗi DUY NHẤT — chuỗi `P86` khớp cả một khối khác nên đổi tên riêng khối này vẫn cho exit 0 với 0 dòng đột biến), Then P86 XANH và in ba con số ngân sách TRÍCH ĐƯỢC từ đúng vế của dòng ngân sách cho cả bản VI lẫn bản EN — số in ra đến từ biến, không từ chữ hằng trong câu.
- AC-6: Given TÁM đột biến — ba cái sửa đúng một vế ngân sách trên bản VI, ba cái sửa đúng vế ấy CHỈ trên bản EN (bản VI để nguyên, vì `kiem` duyệt VI trước rồi dừng ở dòng đỏ đầu tiên nên mọi ca chạm VI đều che mất nhánh EN), và HAI cái đưa GIÁ TRỊ sai vào vế mốc phát hành thay vì xoá vế — bốn ca «xoá vế» bị bắt sớm hơn một bước ngay trong phép trích nên KHÔNG ca nào chạm tới phép so `ship != 1`; thiếu hai ca này thì xoá hẳn nhánh ấy suite vẫn xanh (đo 06/09) (`≤3 lượt/vòng` → `≤9` · `T3 trần 4` → `T3 trần 5` · xoá hẳn vế `mốc phát hành ≤1`), When P86 chạy, Then mỗi ca ĐỎ với CHUỖI GHIM khai trước chứa con số máy trích (ca một ghim `ngan sach luot 9 != so cong 4 - 1`), và thông điệp ấy được IN RA trong bằng chứng; ca xoá vế đỏ vì THIẾU vế, không vì lệch bản chép. Đột biến nào không khai chuỗi ghim là FAIL — assert âm tính trần bị cấm. Và MỌI đột biến TIÊM VĂN BẢN — kể cả ba lần chèn dòng bảng của nhánh thêm-cổng — phải đi qua chân «mũi tiêm có trúng» (ca `them cong o nguon` không tiêm văn bản mà truyền thẳng danh sách nhãn, nên nằm ngoài chân này — xem giới hạn có tên): chuỗi đích xuất hiện đúng một lần và văn bản sau khác trước, nếu không thì một mũi tiêm trượt cũng cho đúng một màu xanh.
- AC-7: Given một đột biến thêm cổng thứ năm vào HAI bản chép (VI + EN) và vào DANH SÁCH nhãn đã phân tích của khối nguồn — không tiêm vào chính khối tsv, xem giới hạn có tên — mà giữ nguyên `≤3`, When P86 chạy, Then ĐỎ với chuỗi ghim `GUIDE.md/QUICKSTART.md (VI): ngan sach luot 3 != so cong 5 - 1` in ra trong bằng chứng — tiền tố bản chép là bắt buộc, thiếu nó thì chuỗi ghim là substring của thông điệp bản EN và đột biến được thoả bởi nhánh EN mà không chứng minh nhánh VI có chạy (đo 06/09: xoá hẳn nửa VI của phép so mà suite vẫn xanh 14/14). Ba bản vẫn khớp nhau nên mọi phép so bản-chép đều xanh: chỉ phép so QUAN HỆ bắt được ca này.
...
- **GIỚI HẠN CÓ TÊN — bằng chứng chỉ giữ BA DÒNG CUỐI của mỗi lệnh.** Cỗ máy verify
  (`feature-loop/workflows/acceptance-verify.js`) cắt output của lệnh PASS xuống ba dòng
  cuối, và các ô dùng chung một lệnh chỉ chạy lệnh ấy một lần. Vì thế nhiều ô của hồ sơ này
  ghim một dòng nằm GIỮA đầu ra (`vế tập-file`, `chiều đỏ 2`, các dòng `P86 MUTANT`) mà bằng
  chứng ghi lại không chứa dòng ấy. Các khẳng định vẫn được máy chạy và vẫn đỏ khi bị phá —
  đã phá thử từng cái trong lượt — nhưng người đọc bằng chứng không tự kiểm lại được. Đây là
  giới hạn của CỖ MÁY chứ không riêng hồ sơ này; sửa đúng tầng là cho mỗi ô một lệnh mà dòng
  cuối chính là khẳng định của nó, và đó là một hồ sơ riêng.

- **GIỚI HẠN CÓ TÊN — hai đột biến «thêm cổng» không tiêm vào khối nguồn.** Ba dòng bảng chèn
  vào hai bản chép là chuỗi markdown gõ tay, không rút từ bộ ghi thật: nếu khuôn bảng đổi, mũi
  tiêm ấy hoá no-op ở tầng khuôn — chân `tiem()` bắt được ca chuỗi đích vắng, nhưng không bắt
  được ca dòng chèn sai khuôn. Nặng hơn: cả hai ca thêm-cổng truyền thẳng DANH SÁCH nhãn đã
  phân tích (`vis + [...]`) thay vì tiêm vào khối tsv nguồn, nên phép đếm cổng của khối nguồn
  không chạy lại. Tiêm thật vào khối tsv thì P86 chết sớm ở chốt «phải có đúng 4 cổng» và không
  bao giờ tới phép so quan hệ — đó là lý do đường tiêm-thật chưa dựng được ở vòng này.
