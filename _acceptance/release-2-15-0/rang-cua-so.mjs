#!/usr/bin/env node
// rang-cua-so.mjs — răng của hồ sơ release-2-15-0, hai chân `vendored` · `viec-meta`.
//
...
//   8  thiếu cờ `--chan vendored|viec-meta`
...
if (chan === 'vendored') {
  ...
  const tep = [...new Set([...khoi[1].matchAll(/\$\{CLAUDE_PLUGIN_ROOT\}\/([A-Za-z0-9._\/-]+)/g)].map(m => m[1]))];
  if (tep.length < 5) stop(2, `FAIL(2): khối chép chỉ rút được ${tep.length} tệp — nghi bộ rút hỏng`);
  ...
  out(`PASS: vendored ${tep.length} tep chep CI (rut tu INIT-CI-COPY-LIST) KHONG doi ke tu lan cat so ${soNeo} (${neo8}) toi cay lam viec (doi chung: cua so tren toan kho co ${toanKho.out.split('\n').length} tep doi)`);
  process.exit(0);
}
