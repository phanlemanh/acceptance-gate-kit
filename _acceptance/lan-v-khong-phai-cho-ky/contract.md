...
- AC-2: Given hồ sơ đủ sáu điều kiện xanh-sạch, chưa ký, When máy quét đọc, Then `state` là `lan-v-mo` nếu `veto_state: mo` có vết giờ parse được — **cửa veto đang mở**, bất kể người có duyệt Cổng 1 hay không (đời thật: `release-2-0-0` có `approved_by` lẫn `mo` ghi ở Cổng 2); còn nếu không có cửa veto mà Cổng 1 do người đóng — `approved_by` có tên, hoặc miễn bằng `gate1_skipped: true` — thì `state` là `xanh-sach`; khoá `mo` không vết không đổi kết luận khi người đã duyệt (ca `nguoi-vet-hong`). Cả hai đều «đã giao», chỉ khác ở chỗ cửa veto còn mở hay không.
- AC-3: Given hồ sơ có `veto_state: mo` + vết giờ + T2 + `verdict: PASS` nhưng **trượt** một trong bốn điều kiện còn lại (`bypass_used` · mục UNCERTAIN · «Known limits» có nội dung · «Ngoài hợp đồng» có nội dung) hoặc **vắng** một trong hai mục, hoặc mục viết bằng tiêu đề h1 (`# Known limits` có nội dung — bộ đọc mục chuẩn chỉ nhận h2–h6, nên h1 không phải mục; máy quét coi là chưa sạch dù lưới đang cho qua, sổ `#9`), When máy quét đọc, Then nó nằm ở `gates: bang-chung` — đây là ca đã làm vòng một trượt; vị từ quay lại khoá vào `veto_state` phải đỏ ở đây.
