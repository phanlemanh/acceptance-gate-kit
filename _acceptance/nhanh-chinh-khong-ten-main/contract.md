...
- Trục «remote khai gì»: KHÔNG có origin → được dò tên quen (AC-1, AC-2) | có origin, hỏi được, giải được (AC-6, AC-7 ô 2) | có origin, hỏi được, KHÔNG giải được → cấm đoán (AC-8). Ô «có origin mà KHÔNG hỏi được» KHÔNG có tiêu chí trong vòng này — xem hạn chế đã khai ở Notes. [thước CE: ba nhánh mà khối dò phân biệt bằng biến `remoteDeclared`] [thước CE: hai vai do chính hợp đồng exit-code của script khai ở đầu file]
...
- Đường remote (phổ biến nhất ở repo tiêu thụ) có phép đo riêng — AC-6; hình dạng cây của bộ CI — AC-7; cấm-đoán-sang-tên-khác — AC-8 (hồi quy r1/r2, đóng ở r4).
...
- **Hạn chế đã biết (owner xếp ngăn tại Cổng Bằng chứng, 29/08; mục thứ ba thêm sau khi thu phạm vi ở S4-r5):**
  - Khi repo CÓ khai remote `origin` mà KHÔNG hỏi được nó (mạng chặn, xác thực
    hỏng, hết trần), máy vẫn rơi về dò bốn tên quen và có thể nhận một tên
    KHÔNG phải nhánh chính thật → mốc so sánh sai, không cảnh báo. Vòng này đã
    thử đóng ở r5 nhưng bản vá lại bịt mất lối thoát `--diff-base`; owner quyết
    THU PHẠM VI, hoàn lại bản vá đó. Lối đi vòng cho người dùng: truyền
    `--diff-base <ref>` tường minh. Lớp này chuyển sang ô
    `khuon-rang-dung-chung`, kèm ghi chú rằng phép bóc tên nhánh còn đọc chữ
    tiếng Anh trong đầu ra của git nên máy đặt ngôn ngữ khác có thể trượt.
