#!/usr/bin/env bash
# E5 (AC-5) — khong-troi-BEN-VIET: chip (3) chi day BEN DOC (12 than lenh);
# ben VIET cau mau (scripts/gate-card.js) va cac vat cua chip (2)/(2)b
# (YOUR-MOVE-BLOCK-TEMPLATE, GATE-INVITE-CLAUSE, GATE-INVITE-SITES) KHONG duoc
# doi mot byte. So voi BASE TUONG MINH origin/main — khong dung `git diff` tran
# (sau commit thi cong no luon rong => chot xanh chan khong, lo P1 chip (2)b).
#
...
# (b) ba vat cua chip (2)/(2)b rut qua marker phai byte-equal giua base va HEAD
...
for M in YOUR-MOVE-BLOCK-TEMPLATE GATE-INVITE-CLAUSE GATE-INVITE-SITES; do
  A="$(printf '%s\n' "$BASE_LAW" | rut "$M")"
  B="$(printf '%s\n' "$HEAD_LAW" | rut "$M")"
  if [ -z "$A" ]; then
    ...
    kêu "$M DA DOI so voi $BASE — vat cua chip (2)/(2)b khong duoc cham"
  fi
done

if [ "$ERR" -eq 0 ]; then
  echo "NO-DRIFT OK (1 diff-check + 3 vat byte-equal so $BASE)"
  exit 0
fi
exit 1
