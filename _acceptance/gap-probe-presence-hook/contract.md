...
- AC-9: Given `gap-probe.md` TỒN TẠI nhưng `verdict` thiếu hoặc nằm ngoài tập `clean|findings|probe-failed`, When approve, Then hook coi như THIẾU file — chặn ở T3, NOTE ở T2 — để một lệnh `touch` rỗng không vượt được chốt fail-stop.

## Coverage

- Trục trạng thái probe: có file verdict clean/findings | vắng file | probe-failed | descope entry | file có nhưng verdict thiếu/rác [state 5 → AC-9: coi như THIẾU, chốt tại Gate 1] [thước CE: tài liệu thiết kế §5 các nhánh cờ của card]
- Trục risk tier: T1 (bỏ qua) | T2 (NOTE) | T3 (chặn) [thước CE: triết lý leo thang dần của GUIDE]
- Trục marker: có `gap_probe_expected` (enforce) | không có (NOTE tối đa — legacy) [thước CE: GUIDE chuẩn F]

## Out of scope

- Enforce ở CI pre-merge — đã có 2 lớp (card + hook), xem entry descope.
