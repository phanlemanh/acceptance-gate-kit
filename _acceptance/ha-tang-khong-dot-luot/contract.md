...
- AC-7: Given ma trận bốn hồ sơ viết trước — (1) `da-cham-boi-thuc-te` có dòng quan sát đủ vế, KHÔNG có `evidence-report.md` · (2) hồ sơ `approved` có dòng sổ nghỉ đủ vế · (3) hồ sơ `draft` sống · (4) hồ sơ `approved` sống — When dựng thẻ bằng `gate-card.js --extract` (cổng tự nhận), Then (1) và (2) ra `routing.hoi` rỗng, không `one_shot`, không `goal_line`, và thẻ HTML có dòng «hồ sơ đã khép … không còn câu hỏi nào cho người»; (3) và (4) ra `routing.hoi` = ["duyệt hay sửa"] như trước vòng (số assert = 4). Chiều đỏ: bản sao `gate-card.js` gỡ vế khép ở nhánh Cổng 1 → hàng (1) và (2) có lại ô hỏi, thông điệp ghim slug.
