...
- AC-2: Given danh sách file khai tường minh — 7 file `commands/*.md`, `skills/acceptance/SKILL.md`, `skills/acceptance/references/human-facing-language.md`, `skills/uat-session/SKILL.md`, `feature-loop/skills/feature-loop/SKILL.md`, `scripts/gate-card.js`, `scripts/evidence-page.js` — When quét với **ranh giới khai tường minh** — token trần = `/<tên>` mà ký tự trước không thuộc `[a-z0-9:/-]` và ký tự sau không thuộc `[a-z0-9:/-]`; `uat-session` thiếu tiền tố = `uat-session` không đứng sau `:`; dòng nằm trong khối marker `COMMAND-NAMES` bỏ qua — Then (i) **0** token trần của 8 tên (`start` `approve` `signoff` `acceptance-card` `acceptance-init` `acceptance-status` `acceptance-report` `feature-loop`) và **0** `uat-session` thiếu tiền tố; (ii) **mọi** token dạng `/<chữ-gạch>:<chữ-gạch>` rút từ 12 file ⊆ cột «lệnh bấm được» của bảng — bản sao chèn `/acceptance-gate:foo` vào `start.md` → đỏ nêu `file:dòng`; (iii) **không rỗng**: số token có tiền tố trên cây mới ≥ 55 (= 48 + 7 baseline) — «sửa bằng cách xoá câu» không xanh; (iv) **đối chứng dương**: cùng hàm quét chạy trên 12 file đọc từ `git show origin/main:<file>` đếm đúng 48 token trần + 7 `uat-session` thiếu tiền tố; (v) chân giữ-gân: chuỗi mẫu `/feature-loop:feature-loop x`, đường dẫn `feature-loop/skills/feature-loop/SKILL.md`, một dòng trong khối bảng → 0 hit; phép đo assert đủ 12 file; bản sao chèn `` `/start` `` → đỏ nêu `file:dòng`; bản sao chèn `uat-session <slug>` → đỏ nêu `file:dòng`.
...
- AC-5: Given thẻ Cổng Phạm vi, When render, Then **không còn** cờ «có ngưỡng/biên nhưng chưa có ca dưới ngưỡng» (fixture contract có AC chứa `≥ 13`) và **không còn** cờ info `--glossary-base`; và trên cây thật, render ba hồ sơ `repo-khai-plugin` · `vao-co-o-ra-co-ten` · `duong-do-trong-dinh-nghia-xong` bằng `gate-card.js` **cũ** (từ `git show origin/main`, đặt đúng chỗ `scripts/gate-card.js` trong một bản sao cây để `require('../lib')` sống) và **mới**, so bằng **TẬP thông điệp cờ**: `cũ ∖ mới` chỉ gồm ba loại (baseline n-a · ngưỡng/biên chưa có ca · --glossary-base), `mới ∖ cũ` = ∅ (không CỘNG cờ lén), và gộp ba hồ sơ mỗi loại xuất hiện ≥ 1 lần trong `cũ` (đối chứng dương: bản cũ thật sự phát ba cờ đó).
