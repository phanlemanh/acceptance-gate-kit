...
- AC-7b: Given **mọi** tài liệu mặt người của repo (`*.md` ở gốc, `commands/*.md`, README của từng plugin), When quét, Then **không file nào** chứa lệnh `claude plugin install|update|marketplace add` **ngoài** khối `GUIDE-PLUGIN-DECLARE` của `GUIDE.md`, và **không dòng nào** gọi một plugin của kit là «tuỳ chọn / tùy chọn / optional / if installed / nếu đã cài» (so sau chuẩn hoá NFC, bắt cả hai chính tả tiếng Việt). Đây là luật LỚP — không danh sách ngoại lệ, không liệt kê tên file.
