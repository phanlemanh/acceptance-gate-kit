...
lời S1 bắt điền trước khi sinh artifact, và bước tra mẫu thị trường có vết.
Không skill mới, không lưới máy nào — phần máy-tự-kiểm cắt trọn 24/08 sau 5
vòng nghiệm thu cùng lớp lỗi; người duyệt soi đặc tả UX tại Cổng Phạm vi.
...
- AC-3: Given `feature-loop/skills/feature-loop/SKILL.md`, When đọc S1, Then có chỉ dẫn: feature chạm UI phải điền `## Đặc tả UX` vào design-doc TRƯỚC khi sinh 3 artifact (resolve khuôn qua resolve-plugin.mjs, KHÔNG hardcode version), contract ghi `design_doc:` và GIỮ marker của khuôn; và nghi thức hình Cổng Phạm vi vẽ hình luồng/màn TỪ section Đặc tả UX; câu «dòng state-matrix» cũ không còn là nguồn thứ hai (trỏ về khuôn).
...
- AC-6: Given bộ ca UX1/UX2-trich/UX3/UX4 của ô này, When đọc test, Then mỗi mệnh đề đo được có mutant đi qua CHÍNH bộ kiểm của chiều xanh (cùng hàm, khác input) và fixture rút TỪ khuôn thật — không assertion âm-tính-một-mình, không fixture viết tay, không chiều đỏ tautology.
...
- Ô Core → AC-1…AC-6; Later/Never → Out of scope. Chân ngành đối chiếu: quy trình UX 8 bước (owner 22/08) + ux-flow-designer · rampstack IA · danh sách khuôn Magdoub [NGÀNH]; mô hình Relume/Mobbin (IA là dữ liệu nguồn) [NGÀNH].
...
- Thước: feature không chạm UI 0 cờ · số từ: ĐẾM TAY trên hồ sơ không-UI trong ván · bảo đảm bởi: AC-4
...
- Không bắt hồ sơ/workspace/consumer cũ migrate — vòng này không thêm lưới nào nên hồ sơ cũ không thể bị cờ.
