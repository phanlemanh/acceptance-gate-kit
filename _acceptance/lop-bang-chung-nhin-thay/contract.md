...
lớp đó (`layer: ui-observed`, neo máy = `executor: ui-check` vì lưới ghi `acceptance-evidence-gate.js` đã giữ bất biến
frame + observed), buộc mỗi hợp đồng có mặt người nhìn có ≥1 eval `ui-check` (theo hợp
