...
- Known limits (Cổng Bằng chứng 08/09, ký: Phan Le Manh — 9 mục ngoài hợp đồng từ rà soát S4-r3, định đoạt «ghi Known limits»; các mục đánh dấu ĐÃ SỬA được đính chính ở lượt chỉ-TRỪ dbe8615d và ghim lại bằng làn máy, giữ đây làm sử liệu): Ngoài-1/4/9 ca PM-LNT-dv5 đo diff so nhánh `main` — ĐÃ SỬA (gỡ khỏi suite); Ngoài-2 ba văn bản nói sai «hook đã bắt screenshot trên block ui-check» — ĐÃ SỬA (đính chính bốn chỗ); Ngoài-3 `LNT_AVAILABLE` xuất mà không bộ đọc nào dùng — còn; Ngoài-5 NOTE pre-merge gộp mọi lỗi node/lib thành «thiếu node hoặc lib» — còn; Ngoài-6 regex chú thích `[ \t]+#` khác các bộ đọc khác (`\s*#`) — còn; Ngoài-7 `html()` trong ca test bỏ mã thoát — còn; Ngoài-8 chiều đỏ có sẵn của LNT1 dựa nhánh `main` — còn. Các mục «còn» gom vào hợp đồng T2 docs+tests trước mốc 2.10.0 cùng hai lỗi đo ở gạch trên.
