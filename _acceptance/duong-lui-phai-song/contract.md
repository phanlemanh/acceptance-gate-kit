...
- AC-6: Given hồ sơ máy-đi-trước (lời mời cổng in ô «veto hay để yên»), When người gõ «veto: <lý do>» ở lệnh ký, Then `veto_state: da-veto` được ghi qua công cụ sửa file và lưới ghi-lúc-viết (`evaluateContractWrite`) CHẤP NHẬN lượt ghi đó trên MỌI hồ sơ máy-đi-trước — ma trận 4 ô khai trước: status ∈ {verified, machine-cleared} × Cổng 1 ∈ {`approved_by` có, làn V `mo`} — kể cả ô machine-cleared × mo mà luật tại mốc cắt nhánh `bafe2aad` (bản base NEO SHA CỐ ĐỊNH của phép so, không phải ref trôi) từ chối với thông điệp Cổng-1-chưa-được-ghi; lối này chỉ mở cho status ∈ {verified, machine-cleared} có bản cũ đang `mo` — hồ sơ mới/draft hay approved/signed-off kèm da-veto vẫn theo luật cũ; sổ quyết định có entry `type: veto` mang lý do nguyên văn và `decided_by`, commit `Veto: <slug> — <tên>`, máy in một dòng rồi DỪNG; lưới trước-merge chặn merge cho tới khi người xử.
