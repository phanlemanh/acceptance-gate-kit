...
- AC-2 (đổi khuôn, owner 08/09): Given hồ sơ đi làn V xanh-sạch, When lưới trước-merge chạy, Then hồ sơ đó KHÔNG `continue` sau NOTE xanh-sạch mà đi CÙNG chuỗi kiểm với hồ sơ có chữ ký — hoá cũ, pin ma, re-pin provenance, làn eval, soi lại — và kết bằng dòng OK gọi tên làn V; cụ thể, ma trận 8 ô khai trước: (1) trong diff PR, file ngoài `_acceptance/` và ngoài T1 đổi sau `verified_commit` → `VIOLATION [<slug>]: evidence is stale — code changed after verify (verified_commit <sha>)` CÙNG câu với hồ sơ có chữ ký; (2) không file nào đổi → NOTE xanh-sạch rồi OK làn V, 0 VIOLATION; (3) `verified_commit` là SHA không có trong repo (clone đầy đủ) → VIOLATION pin-ma P184; (4) `verified_commit` rỗng → NOTE «report has no verified_commit» (đường đọc-cũ, cùng câu với hồ sơ có chữ ký); (5) hồ sơ ngoài diff PR → không soi stale (stale-theo-diff-pr giữ nguyên); (6) KHÔNG có `--base` (DIFF_READY=0) mà cây đổi → vẫn kiểm-tất, VIOLATION stale (không tắt im); (7) làn suite-only chống lưng pin (không `evals_exit`) → luật làn eval cắn «recorded no evals_exit»; (8) run_id báo cáo không có trong sổ chạy của hồ sơ → soi lại strict cắn. Chiều đỏ: bản sao TRẢ LẠI `continue` → ô (1) và (7) đều 0 VIOLATION.
...
- AC-6: Given hồ sơ máy-đi-trước (lời mời cổng in ô «veto hay để yên»), When người gõ «veto: <lý do>» ở lệnh ký, Then `veto_state: da-veto` được ghi qua công cụ sửa file và lưới ghi-lúc-viết (`evaluateContractWrite`) CHẤP NHẬN lượt ghi đó trên MỌI hồ sơ máy-đi-trước — ma trận 4 ô khai trước: status ∈ {verified, machine-cleared} × Cổng 1 ∈ {`approved_by` có, làn V `mo`} — kể cả ô machine-cleared × mo mà luật tại mốc cắt nhánh `bafe2aad` (bản base NEO SHA CỐ ĐỊNH của phép so, không phải ref trôi) từ chối với thông điệp Cổng-1-chưa-được-ghi; lối này chỉ mở cho status ∈ {verified, machine-cleared} có bản cũ đang `mo` hoặc đã `da-veto` (ghi lại hồ sơ đang veto — Ngoài-3, owner nâng phạm vi 08/09) — hồ sơ mới/draft hay approved/signed-off kèm da-veto vẫn theo luật cũ; sổ quyết định có entry `type: veto` mang lý do nguyên văn và `decided_by`, commit `Veto: <slug> — <tên>`, máy in một dòng rồi DỪNG; lưới trước-merge chặn merge cho tới khi người xử.
...
- AC-8: Given hồ sơ `status: verified`, `risk_tier: T2`, đủ sáu điều kiện xanh-sạch, When chạy `scripts/khong-can-nguoi.mjs --write --root <r> --slug <s>`, Then chỉ dòng `status:` của hợp đồng đổi thành `machine-cleared`, script tự kiểm bằng `evaluateContractWrite` và thoát 0; Given thiếu một điều kiện hoặc hạng T3, Then thoát 2 nêu điều kiện trượt đầu tiên và file không đổi byte nào; `--check` không ghi. Lệnh hỏi CẢ HAI luật của lưới ghi-lúc-viết: `evaluateContractWrite` và `machineClearedSignoffConflict` — báo cáo đã có `human_signoff` thì exit 2 cùng câu với lưới, không ghi; dòng `status: verified` mang comment đuôi theo khuôn hợp đồng vẫn ghi được, comment giữ nguyên.
...
- AC-11: Given cây thật sau khi vá, When chạy lưới trước-merge ở chế độ soi toàn bộ hồ sơ so với `origin/main`, Then số VIOLATION NGOÀI họ nợ có tên (làn suite-only, dòng «recorded no evals_exit» — sổ nợ hai chiều `SUITE_ONLY_LANE_DEBT` của kit) không tăng so với `origin/main`, và mọi dòng VIOLATION tăng thêm đều là dòng nợ đó của một slug có tên trong sổ (làn V nay cũng bị luật làn eval soi nên nợ của nó lộ ra — đúng luật 7d12ffad, không phải lỗi mới); luật soi-lại-câm không cắn hồ sơ nào của kit; răng additive-only (DV5) xanh với mọi dòng gỡ khai đích danh (gồm dòng `continue` của làn V).
...
Known limits (Cổng Bằng chứng 08/09/2026, owner định đoạt «ghi Known limits»):
- Ngoài-1: luật làn eval không còn mốc ngày lẫn phạm vi diff (adef480c + 7d12ffad, owner đẩy thẳng) — lưới của chính kit đỏ 49 dòng nợ làn suite-only ở mọi lượt cho tới chiến dịch ghim lại; CLAUDE.md chưa khai ngoại lệ cho hai bất biến «đường đọc-cũ» và «re-pin theo release» — hai nguồn luật đang cãi nhau, xử ở mốc phát hành kế cùng chiến dịch.
- Ngoài-2: `khong-can-nguoi.mjs --write` chưa kiểm hình dạng `--slug`/`--root` như `gate-card.js` (`../` có thể ghi ra ngoài `_acceptance/`); công cụ nội bộ, người vận hành chạy — chưa khắc phục.
- Ngoài-5: ô «thiếu cờ → exit 3» của chân ket-ghi chỉ ghim mã thoát, chưa ghim câu usage.
