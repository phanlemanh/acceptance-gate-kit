...
- **Known limits (Cổng 2 round 3, Manh quyết ký-ngay):** (3) chân đột biến của P86/P87 là tautology — `replace`-all rồi assert not-in luôn đúng, bản đột biến không được đưa lại vào phép đo; pin dương vẫn grep file thật nên không có false-green đang sống, sửa chân âm ở feature kế chạm `tests/plugins/` (đúng lớp bất biến số 1 CLAUDE.md); (4) văn evals khai quá phép đo thật: E6/E7/E10 ghi đối-chứng-âm chưa được cài, E12 ghi "bằng node" trong khi P88 so semver bằng python tuple (đúng ý numeric, sai chữ) — chỉnh cùng lượt với (3).
