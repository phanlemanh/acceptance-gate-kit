...
- AC-1: Given kho nguồn của kit, When đọc `skills/acceptance/references/human-facing-language.md`, Then file tồn tại, chứa đủ sáu luật mở đầu bằng đúng mã `N1`…`N6` kèm nội dung, hai phép thử gọi đích danh tên `Xoá-tên-máy` và `Người-thứ-ba`, và một bảng ví dụ TRƯỚC/SAU phủ cả sáu luật (mỗi luật ít nhất một cặp).
...
- AC-8: Given file tham chiếu, When rút khối giữa cặp marker `PLAN-SUMMARY-TABLE-TEMPLATE`, Then chỉ có đúng một cặp marker trong toàn kho nguồn và khối rút được là một bảng ba cột mang đúng ba tên cột đã chốt, kèm ít nhất một dòng ví dụ.
- AC-9: Given khuôn bảng rút ra từ marker (không viết tay), When đọc nó bằng luật tách ô bảng markdown của kit, Then đọc ra đúng ba tiêu đề cột và mỗi dòng ví dụ có đúng ba ô; bản bị đột biến (thiếu cột, thừa cột, hoặc nhồi nhiều việc vào một ô) phải ĐỎ với thông điệp riêng của từng đột biến.
- AC-10: Given toàn bộ cây nguồn (trừ mirror `plugins/`, `_acceptance/`, `tests/`), When đếm số file chứa tên ba cột, Then đúng một file chứa chúng — khuôn có một chỗ duy nhất, các nơi dùng gọi theo tên marker.
