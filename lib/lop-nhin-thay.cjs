...
//
// Neo máy của nghĩa vụ là `executor: ui-check` (hook schema v2 đã bắt mọi block ui-check phải có
// screenshot: + observed:). `layer: ui-observed` là NHÃN KHAI để luật ghép cặp đọc đối xứng với
...
function tokensOf(surfaces) {
  const s = String(surfaces == null ? '' : surfaces)
    .replace(/^\s*surfaces:\s*/i, '')
    .replace(/[ \t]+#.*$/, '')
    .replace(/^#.*$/, '');
...
function frontLine(text, key) {
  ...
  return l ? l.slice(key.length + 1).replace(/[ \t]+#.*$/, '').trim() : '';
}
