...
/**
 * `paths:` của TỪNG eval — chấp cả hai dạng YAML kho này viết thật:
 *   paths: [a/**, "b/c.js"]      (inline)
 *   paths:\n      - a/**         (block sequence)
 * Trả { <id>: string[] } — eval không khai `paths` thì VẮNG hẳn khỏi map (khác với
 * mảng rỗng: bên đọc phải phân biệt «không khai» với «khai rỗng»).
 * @param {string} text nội dung evals.yaml
 */
function pathsOf(text) {
  const out = {};
  let cur = null; let pending = false; let indent = 0;
  let blk = null; // block scalar đang mở: { indent }
  const unq = s => s.trim().replace(/^["']|["']$/g, '');
  for (const raw of String(text == null ? '' : text).split('\n')) {
    const line = raw.replace(/\t/g, '  ');
    // Luật 2 của file này (dòng THÂN block không bao giờ được quét key) áp cho CẢ pathsOf:
    // một dòng `paths: [...]` nằm trong thân `expected: >` là DỮ LIỆU của lời khai, không
    // phải lời khai. Bỏ luật này thì staleScope đọc được paths cho eval KHÔNG khai paths →
    // thu phạm vi hoá-cũ trên một hồ sơ lẽ ra phải đi luật cũ, tức fail-open ở đúng chỗ
    // K4 hứa không có đường fail-open (finding S4-r2).
    if (blk) {
      if (!line.trim()) continue;
      if (line.search(/\S/) > blk.indent) continue;
      blk = null; // dedent → dòng này là dòng thường
    }
    const idM = line.match(/^\s{0,4}-\s+id:\s*(\S+)/);
    if (idM) { cur = unq(idM[1]); pending = false; continue; }
    if (!cur) continue;
    const bm = line.match(/^(\s*)[A-Za-z_][A-Za-z0-9_-]*:\s*(.*)$/);
    if (bm && BLOCK_RE.test(bm[2].trim())) { blk = { indent: bm[1].length }; pending = false; continue; }
    if (pending) {
      const item = line.match(/^(\s+)-\s+(.*)$/);
      if (item && item[1].length > indent) { out[cur].push(unq(item[2])); continue; }
      if (line.trim()) pending = false;
    }
    const f = line.match(/^(\s+)paths:\s*(.*)$/);
    if (!f) continue;
    const v = stripComment(f[2]);
    if (v.startsWith('[')) out[cur] = v.replace(/^\[|\]$/g, '').split(',').map(unq).filter(Boolean);
    else if (v === '') { out[cur] = []; pending = true; indent = f[1].length; }
  }
  return out;
}

module.exports = { parseEvals, BLOCK_RE, stripComment, pathsOf };
