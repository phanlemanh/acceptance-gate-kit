...
// lượt; không mục nào có thì quét CẢ TỆP (giữ đúng hành vi cũ của acBlindSpot).
// So khớp của sectionLines KHÔNG phân biệt hoa thường, nên `Acceptance criteria`
// trùng `Acceptance Criteria` và mục thứ ba chỉ là trang trí — bỏ, kẻo dạy sai
// người sau rằng danh sách này phân biệt hoa thường (lượt chấm 4).
const CRITERIA_HEADINGS = ['Criteria', 'Acceptance Criteria'];
function criteriaLines(contractText) {
  // LÙI CHỈ KHI MỤC RỖNG — bất biến cũ, khôi phục sau lượt chấm 4.
  // Bản trước nới thành «mục không có dòng nào trông như tiêu chí», và một hợp
  // đồng có `## Criteria` viết bằng BẢNG rơi ngay vào nhánh quét-cả-tệp: thẻ Cổng
  // Phạm vi hiện AC-4/AC-9 lấy từ mục «Known limits» (tiêu chí MA), còn bộ dò trả
  // null nên không cờ nào kêu. Đúng thứ bình luận của acBlindSpot ở trên đã cấm:
  // «Whole-file everywhere over-counts … Whole-file only when section() came back
  // empty». Tái hiện được bằng một lệnh; xem sổ quyết định.
...
module.exports = { AC_LINE, AC_SUSPECT, AC_XREF, AC_HEAD, CRITERIA_HEADINGS, criteriaLines, parseAC, parseACBlock, acBlindSpot, blindSpotText };
