...
function criteriaLines(contractText) {
  for (const h of CRITERIA_HEADINGS) {
    ...
    // Đòi mục CHỨA ít nhất một dòng trông-như-tiêu-chí, không chỉ đòi «khác rỗng».
    // Hợp đồng thật dạng h2 (media-library/embed-on-approve và 17 hồ sơ khác) đặt
    // `## AC-n` làm cấu trúc TOP-LEVEL, không có mục bao ngoài; một mục Criteria
    // chỉ chứa dòng trắng hoặc một câu dẫn sẽ nuốt hết phạm vi và giấu mọi tiêu chí.
    if (ls.some(x => AC_SUSPECT.test(x.l) && !AC_XREF.test(x.l))) return ls;
  }
