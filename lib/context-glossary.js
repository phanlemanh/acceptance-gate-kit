...
// Require CHÉO giữa file lib/ phải GUARD: repo tiêu thụ vendored TỪNG file, một cây thiếu
// md-section.cjs mà ném lúc nạp module sẽ làm cả W6 biến mất lặng lẽ (hai call site đều
// nuốt lỗi require). Nếp có sẵn: evidence-core.loadEvalYaml, nguong-o-co-hoi.LNT.
let sectionLines = null;
try { ({ sectionLines } = require('./md-section.cjs')); } catch (_) { sectionLines = null; }
 ...
 */
function findViolations(docText, glossary, opts) {
  if (!glossary || !glossary.aliases.length) return [];
  let prose = stripNonProse(docText);
  // (1) PHẠM VI: `opts.section` → chỉ chấm chữ trong section đó, phần còn lại bị làm
  // trắng (GIỮ độ dài để lineNo không lệch). Vì sao: luật W6 nói về CÁCH PHÁT BIỂU
  // TIÊU CHÍ; Context/Notes là chỗ kể chuyện, trích lời, khai giới hạn — bắt từ ở đó
  // sinh ra 127 dòng không ai xử trên chính kho này (đúc kết 08/09), và luật bị bỏ
  // qua hàng loạt là luật đang đo sai chỗ.
  // Thiếu md-section.cjs → KHÔNG thu phạm vi được: chấm CẢ tài liệu (chặt hơn) và nói ra,
  // thay vì im lặng bỏ luật. Bên gọi in dòng note nếu muốn.
  if (opts && opts.section && typeof sectionLines !== 'function') {
    if (opts.onDegraded) opts.onDegraded('thiếu lib/md-section.cjs — W6 chấm cả tài liệu, không thu về section');
  } else if (opts && opts.section) {
    const keep = new Set(sectionLines(docText, opts.section).map(x => x.no));
    prose = prose.split('\n').map((l, i) => (keep.has(i + 1) ? l : l.replace(/[^\n]/g, ' '))).join('\n');
  }
  // (2) ĐỊNH DANH ASCII gạch nối (`ui-check`, `pre-merge`) là MỘT từ: che nó đi trước
  // khi dò alias, trừ khi chính token đó là alias/allow/term. Ghép gạch nối TIẾNG VIỆT
  // («check-lại», «thẻ-cổng-2») KHÔNG phải định danh — alias bên trong vẫn phải kêu,
  // nếu không thì luật tự mù trước cách viết phổ biến nhất của kho này.
  {
    const keepWords = new Set();
    for (const a of glossary.aliases) keepWords.add(String(a.alias).toLowerCase());
    for (const t2 of glossary.terms) keepWords.add(String(t2.term).toLowerCase());
    for (const a of glossary.allow) keepWords.add(String(a).toLowerCase());
    prose = prose.replace(/(?<![\p{L}\p{N}_-])[a-z0-9]+(?:-[a-z0-9]+)+(?![\p{L}\p{N}_-])/giu,
      m => (keepWords.has(m.toLowerCase()) ? m : m.replace(/[^\n]/g, '.')));
  }
  const allowSpans = [];
