...
function walkEvalExits(payload) {
  ...
  for (const line of String(payload).split('\n')) {
    ...
    if (cur) {
      ...
      if (m.index === leading) byEval.set(cur, code); // từ khoá là token đầu dòng — dòng khai thật
      // else: từ khoá nằm giữa một dòng khác (ghi chú tự do) — bỏ qua
    } else if (code !== 0) {
      outside.push(code);
    }
  }
  return { byEval, outside };
}
...
function evaluateEvidence(payload, opts) {
  ...
  // ở đây tự lọc `code !== 0` trước khi xét vi phạm.
  const { byEval: blockExits, outside: outsideExits } = walkEvalExits(payload);
