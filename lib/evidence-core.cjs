...
//                khớp EXIT_LINE_RE VÀ từ khoá neo ngay đầu phần không-trắng
//                của dòng (token đầu dòng, sau khoảng trắng đầu dòng). Đây là
//                MỘT ngoại lệ duy nhất hợp đồng cho phép.
//   outside    — mảng các mã thoát KHÁC 0 khớp EXIT_LINE_RE nằm ngoài mọi
//                khối eval, theo đúng thứ tự xuất hiện.
//   violations — mảng {evalId, code, line} cho dòng TRONG khối khớp
//                EXIT_LINE_RE mang mã KHÁC 0 mà từ khoá KHÔNG neo đầu dòng —
//                dấu gạch đầu dòng chen trước ("  - exit_code: 9"), lệch thụt
//                lề, hay nằm giữa một câu văn xuôi ("note: ... exit_code: 9
//                ...") đều CÙNG một hình dạng vi phạm, không còn phân biệt
//                theo "trông giống trường thật" hay "trông giống ghi chú" —
//                thứ duy nhất phân biệt là VỊ TRÍ neo, không phải nội dung
//                xung quanh. Cùng bộ lọc "khác 0" như `outside`: L1
//                CONSISTENCY chỉ soi mã THẤT BẠI — một mã 0 lạc chỗ (vd
//                "... exit=0 ...") không tố cáo điều gì nên không phải vi
//                phạm (corpus thật, hồ sơ lenh-tran-tai-lieu-dau-tay, có
//                đúng hình dạng này ở một ghi chú vô hại).
function walkEvalExits(payload) {
  ...
  for (const line of String(payload).split('\n')) {
    ...
    if (cur) {
      ...
      if (m.index === leading) {
        byEval.set(cur, code); // neo đầu dòng — dòng trường thật DUY NHẤT được tha
      } else if (code !== 0) {
...
function evaluateEvidence(payload, opts) {
  ...
  // ở đây tự lọc `code !== 0` trước khi xét vi phạm. `violations` là MỌI dòng
  // TRONG khối khớp EXIT_LINE_RE mà từ khoá không neo đầu dòng — fail-CLOSED,
  // không được rơi mất, và không còn nhánh nào tha nó theo hình dạng.
