...
// the message. Returns { errs } — messages carry NO prefix so recheck
// ("REPIN x ") and pre-merge ("VIOLATION [slug]: ") print one text.
function checkRepinEvals(entry, evalsText, slug) {
  ...
  const red = ids.filter(i => has(i) && ex[i] !== 0);
  ...
  if (red.length) errs.push(`re-pin lane "${id}" evals_exit has nonzero exit for ${red.map(i => `${i}=${JSON.stringify(ex[i])}`).join(', ')} — a red eval cannot back a pin; fix the eval and run a NEW lane`);
  return { errs };
}
...
function evaluateContractWrite(newPayload, oldPayload) {
  ...
  if (v.present && v.state === 'da-veto') {
    ...
    const laneStatus = status === 'verified' || status === 'machine-cleared';
...
// ── Phạm vi hoá-cũ theo `paths:` của eval (gom-duc-ket-2-10-0, K4) ──────────────
// Luật cũ đo «cây có đổi không», nên mỗi lần nhánh chính chạm `tests/` là mọi hồ sơ đã
// ký hoá cũ và phải ghim lại — 277 lượt trên kho này, 112 ở oneflow, 46 ở map; ba lượt
// trong MỘT ngày, gần một giờ làn máy, 0 thông tin về vật. Mỗi eval đã khai `paths` là các
// file nó THẬT SỰ kiểm; dùng chính lời khai đó để thu phạm vi.
//
// FAIL-SAFE VỀ PHÍA CHẶT: chỉ thu khi MỌI eval chạy-bằng-máy (test/script/ui-check) đều
// khai `paths`. Một eval thiếu → trả null → bên gọi giữ nguyên luật cũ (cả cây). Eval
// `judgment` không chạy trên cây nên không tính.
// GIỚI HẠN ĐÃ KHAI: `paths` là khai báo TĨNH, KHÔNG bắc cầu qua require/import — hồ sơ
// khai thiếu file mà vật được đo nạp vào có thể «tươi giả». Ngưỡng mở lại ghi ở hồ sơ.
const STALE_SCOPE_EXECUTORS = ['test', 'script', 'ui-check'];
function staleScope(evalsText) {
  const yaml = loadEvalYaml();
  if (!yaml || typeof yaml.pathsOf !== 'function') return null;
  // Normalize đi qua stripComment — MỘT nguồn cắt chú thích với mọi bộ đọc dòng khác.
  // Không có nó thì `executor: test  # chay jest` cho ra chuỗi lạ, eval rơi khỏi danh sách
  // máy và vừa không bị đòi khai paths vừa không góp paths — fail-open ở đúng chỗ hàm này
  // tuyên bố không có (finding S4-r1/r3).
  const norm = typeof yaml.stripComment === 'function'
    ? (s => yaml.stripComment(s).replace(/^["']|["']$/g, '').trim())
    : (s => String(s).trim());
  const evals = yaml.parseEvals(String(evalsText == null ? '' : evalsText), ['executor'], norm);
  if (!evals.length) return null;
  const declared = yaml.pathsOf(evalsText);
  const globs = [];
  for (const e of evals) {
    const ex = String(e.executor || '').trim().toLowerCase().replace(/^["']|["']$/g, '');
    // FAIL-CLOSED: executor không nhận diện được (viết lạ, typo, khoá mới) → KHÔNG im lặng
    // bỏ qua eval đó, mà trả null để cả hồ sơ về luật cũ. Bỏ qua là đường fail-open: một
    // eval máy tàng hình vừa thoát nghĩa vụ khai paths vừa không góp paths vào phạm vi.
    if (!ex) return null;
    if (!STALE_SCOPE_EXECUTORS.includes(ex) && ex !== 'judgment') return null;
    if (!STALE_SCOPE_EXECUTORS.includes(ex)) continue;
    const p = declared[e.id];
    if (!Array.isArray(p) || !p.length) return null;   // thiếu khai → luật cũ
    globs.push(...p);
  }
  if (!globs.length) return null;                       // không eval máy nào → luật cũ
...
// CLI cho bash (pre-merge): `node lib/evidence-core.cjs stale-scope <evals.yaml>` →
// mỗi glob một dòng, exit 0; hồ sơ không đủ khai → exit 3 (bên gọi giữ luật cũ).
if (require.main === module) {
  const [cmd, file] = process.argv.slice(2);
  if (cmd !== 'stale-scope' || !file) { process.stderr.write('usage: evidence-core.cjs stale-scope <evals.yaml>\n'); process.exit(2); }
  let txt = '';
  try { txt = require('fs').readFileSync(file, 'utf8'); } catch (_) { process.exit(3); }
  const scope = staleScope(txt);
  if (!scope) process.exit(3);
