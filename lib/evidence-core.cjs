...
// b'` giải ra thiếu dấu đóng → `bash -c` thoát 2 → luật `expected_exit` (ship
  ...
  for (const line of lines) {
    ...
    if (m[1] === parts[depth]) {
      if (depth === parts.length - 1) {
        const val = unquoteScalar(m[2].replace(/\s+#.*$/, '').trim());
...
function resolveConfigList(configText, dottedKey) {
  ...
  for (const raw of lines) {
    ...
    if (leafIndent >= 0) {
      ...
      if (m) out.push(unquoteScalar(m[1].replace(/\s+#.*$/, '').trim()));
      continue;
    }
    if (indent < expectedIndent) {
    ...
    if (m[1] === parts[depth]) {
      if (depth === parts.length - 1) {
        ...
        if (val.startsWith('[')) return val.replace(/^\[|\]$/g, '').split(',').map(s => unquoteScalar(s.trim())).filter(Boolean);
