...
function mucChoNguoi({ findingsText, ledgerText }) {
  ...
  if (findingsText == null) {
    return { n: 0, ngoai: 0, trong: 0, daDinhDoat: 0, docDuoc: true, vangTep: true, why: '' };
  }
  const r = bo.parse(String(findingsText));
  const ngoai = (r.findings || []).length;
  const trong = (r.inContract || []).length;
  // ĐẾM THEO NHÃN, không đếm DÒNG (sửa sau lượt chấm 1). Đếm mọi dòng
  // `stage: "gate2"` là sai nặng và sai NGƯỢC chiều an toàn: trên 22 kho có 621
  // dòng như thế với 12 loại `type` khác nhau (approach · descope · revisit ·
  // fix · seal · signoff · verdict …), phần lớn KHÔNG định đoạt mục rà soát nào.
  // Một hồ sơ 1 mục treo + 1 dòng veto ở Cổng 2 sẽ ra n = 0, tức SẠCH GIẢ, đúng
  // lỗ vòng này đi bịt.
  // Thẻ đánh số mục bằng nhãn tra cứu «Ngoài-n» / «Trong-n», và người định đoạt
  // trả lời theo nhãn đó — 153/621 dòng gate2 trên kho thật có nhắc nhãn. Đếm
  // TẬP nhãn phân biệt: dòng không nhắc nhãn nào không trừ gì cả.
  const nhan = new Set();
  for (const l of String(ledgerText == null ? '' : ledgerText).split('\n')) {
    if (!l.trim()) continue;
    let o; try { o = JSON.parse(l); } catch (_) { continue; }
    if (!o || String(o.stage) !== 'gate2') continue;
    for (const m of JSON.stringify(o).matchAll(/\b(Ngo[àa]i|Trong)-(\d+)\b/g)) nhan.add(`${m[1]}-${m[2]}`);
  }
  const daDinhDoat = nhan.size;
  return { n: Math.max(0, ngoai + trong - daDinhDoat), ngoai, trong, daDinhDoat, docDuoc: true, why: '' };
}
...
function dieuKienFindings({ findingsText, ledgerText, reportText }) {
  ...
  const goc = `${m.ngoai}+${m.trong} mục, ${m.daDinhDoat} đã định đoạt`;
