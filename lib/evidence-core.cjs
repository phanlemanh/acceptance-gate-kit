...
const EXIT_LINE_RE = /(?:exit_code|verifier_exit_code|exit)\s*[:=]\s*(-?\d+)\b/i;
...
// đóng khối.
//   byEval    — Map<evalId, mã thoát> ghi trong khối của từng eval (dòng
//               khớp EXIT_LINE_RE VÀ từ khoá đứng ngay sau khoảng trắng đầu
//               dòng — dòng ghi chú lẫn trong khối bị bỏ qua).
//   outside   — mảng các mã thoát KHÁC 0 khớp EXIT_LINE_RE nằm ngoài mọi khối
//               eval, theo đúng thứ tự xuất hiện.
//   malformed — mảng {evalId, code, line} cho dòng TRONG khối mà từ khoá
//               KHÔNG đứng đầu dòng nhưng cũng KHÔNG phải ghi chú tự do: phần
//               trước từ khoá chỉ là khoảng trắng + một dấu gạch đầu dòng
//               (hình dạng của một trường thật, ví dụ "  - exit_code: 9",
//               chỉ lệch thụt lề/kiểu gạch đầu dòng). Ca này từng bị hai
//               nhánh còn lại bỏ sót hoàn toàn — không vào byEval (từ khoá
//               không ở token đầu dòng), không vào outside (đang trong khối)
//               — nên biến mất khỏi phép kiểm (lỗ fail-open, hồ sơ
//               eval-khai-ma-thoat-mong-doi 2026-09-09). Phân biệt với ghi
//               chú tự do ("note: last run had exit_code: 9 ...") bằng CHÍNH
//               phần văn bản trước từ khoá: chỉ trắng+"-"+trắng thì là hình
//               dạng trường thật lệch định dạng; có chữ khác thì là văn xuôi
//               và vẫn được tha như trước.
function walkEvalExits(payload) {
  ...
  for (const line of String(payload).split('\n')) {
    ...
    const m = line.match(EXIT_LINE_RE);
    if (!m) continue;
    const code = Number(m[1]);
    if (cur) {
      const leading = line.length - line.trimStart().length;
      if (m.index === leading) {
        byEval.set(cur, code); // từ khoá là token đầu dòng — dòng khai thật
      } else if (/^\s*-\s*$/.test(line.slice(0, m.index))) {
        // dòng lệch định dạng: hình dạng một trường thật (dấu gạch đầu dòng
        // chen trước từ khoá) — không xác định được đây có phải mã thật của
        // khối hay không nên fail-CLOSED, không được rơi mất như trước.
        malformed.push({ evalId: cur, code, line });
      }
      // else: từ khoá nằm giữa một dòng ghi chú tự do (văn xuôi) — bỏ qua
    } else if (code !== 0) {
      outside.push(code);
    }
  }
  return { byEval, outside, malformed };
}

// Mã thoát ghi TRONG khối của từng eval. Lớp mỏng gọi vào walkEvalExits —
// giữ nguyên chữ ký và hành vi cũ vì checkRepinEvals đang dùng. Dùng cho luật
// hai vế của re-pin và (gián tiếp, qua walkEvalExits) cho L1 CONSISTENCY.
function extractEvalBlockExits(payload) {
  return walkEvalExits(payload).byEval;
}
...
function evaluateEvidence(payload, opts) {
  ...
  // 2026-09-09). `outside` đã lọc sẵn mã 0; `byEval` giữ MỌI mã (kể cả 0) nên
  // ở đây tự lọc `code !== 0` trước khi xét vi phạm. `malformed` là dòng
  // TRONG khối mà từ khoá không đứng đầu dòng NHƯNG cũng không phải ghi chú
  // tự do (xem walkEvalExits) — fail-CLOSED, không được rơi mất.
  const { byEval: blockExits, outside: outsideExits, malformed: malformedExits } = walkEvalExits(payload);
  let consistencyFailure = null;
  {
    ...
    for (const { evalId, code, line } of malformedExits) {
      viPham.push(
        `eval ${evalId}: dòng lệch định dạng "${line.trim()}" mang mã ${code} — dấu gạch đầu dòng chen trước từ khoá exit_code nên không xác định được đây có phải mã thật của khối hay không; sửa lại đúng thụt lề (bỏ dấu "-" trước exit_code) hoặc xoá dòng nếu là thừa`
      );
    }
