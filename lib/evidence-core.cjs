...
// From REPIN_EVALS_SINCE every lane that backs verified_commit must carry
// `evals_exit` = {evalId: exit} covering every test/script eval declared in
// the slug's evals.yaml, all 0 (ui-check/judgment cannot run in a machine
// lane — declared limit, see GUIDE §7.1). Lanes dated before the mark are
// suite-only history (~530 citations in the kit's own corpus alone, signed
// records are immutable): the readers report them, never block. A lane that
// DID record evals_exit is always judged, whatever its ts — it claims eval
// evidence, so the claim is checked. A lane with no parseable ts cannot claim
// to be history — fail-closed.
const REPIN_EVALS_SINCE = '2026-09-07T12:00:00Z';
...
function repinLaneIsHistory(entry) {
  if (entry && Object.prototype.hasOwnProperty.call(entry, 'evals_exit')) return false;
  const t = entry && typeof entry.ts === 'string' ? Date.parse(entry.ts) : NaN;
  return Number.isFinite(t) && t < Date.parse(REPIN_EVALS_SINCE);
}
...
// the message. Returns { history, note?, errs } — messages carry NO prefix so
// recheck ("REPIN x ") and pre-merge ("VIOLATION [slug]: ") print one text.
function checkRepinEvals(entry, evalsText, slug) {
  ...
  if (repinLaneIsHistory(entry)) {
    return { history: true, errs: [], note: `re-pin lane "${id}" (ts ${ts}, before ${REPIN_EVALS_SINCE}) is suite-only — this slug's evals were NOT re-run at ${sha}; grandfathered, re-pin with the eval lane at the next campaign` };
  }
  const errs = [];
  if (evalsText == null) {
