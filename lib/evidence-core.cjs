...
function evaluateContractWrite(newPayload, oldPayload) {
  ...
  if (status === 'machine-cleared' && hangMoi === 'T3') {
    failures.push(`status: machine-cleared on a T3 contract — «machine-cleared» means Gate 2 passed WITHOUT a human signature, and that is T2-only. T3 touches enforcement core / data / breaking API, so Gate 2 ALWAYS stops for a human: use verified, then /acceptance-gate:signoff to reach signed-off.`);
  }
