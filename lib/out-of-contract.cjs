...
// viết vẫn in nó (acceptance-verify.js sinh cả ba tiêu đề), còn thẻ và luật
// xanh-sạch đều không đọc. Hệ quả đo được: một hồ sơ dừng-vá mang lỗi có mã AC
// chưa sửa mà không bề mặt nào của người quyết nhìn thấy.
...
// Mục «Trong hợp đồng»: gạch đầu dòng · tiêu đề con · hoặc dòng `khoá: giá trị`
// ở BẤT KỲ cột nào. Một dòng khai rỗng dạng «(không có — …)» KHÔNG khớp, nên nó
// không kêu oan trên hồ sơ lành, cùng nếp với vế của mục «Ngoài hợp đồng».
const IN_TRIED_ITEM_RE = /^\s*[-*]\s+\S|^#{3,6}\s+\S|^\s*[A-Za-zÀ-ỹ][^:]{0,40}:\s*\S/;
...
function parse(text) {
  ...
  return {
    ...
    findings,
    // Bóc bằng CHÍNH parseFindings của mục «Ngoài hợp đồng»: hai mục dùng chung
    // khuôn OOC-ITEM-TEMPLATE ở bên viết, nên hai bộ bóc là hai bản sao sẽ trôi.
    inContract: inLines ? parseFindings(inLines) : [],
    unclassified,
    cluster: clusterLine ? clusterLine[1].trim() : null,
    // Vế THỨ HAI của cùng bộ dò, cho mục «Trong hợp đồng» (owner duyệt 13/09).
    // Trước bản này mục ấy KHÔNG có bộ dò nào: 103 hồ sơ trên 22 kho có mục đó
    // mang chữ thật mà bóc ra 0, và cả hai bề mặt người quyết đều im — thẻ không
    // render khối nào, điều kiện xanh-sạch cộng 0 rồi gọi là sạch. Đúng lớp lỗi
    // `suspect_empty` sinh ra để chặn, chỉ tái sinh ở mục NẶNG HƠN.
    // «Đã thử viết mục» ở mục này RỘNG hơn `OOC_TRIED_ITEM_RE`: khuôn đời trước
    // viết `### tiêu đề` + `- file:` ở cột 0, không thụt lề, nên luật cũ (đòi `**`
    // hoặc dòng thụt lề `khoá: giá trị`) trượt hết — đo trên 22 kho: bắt 0/103.
    suspect_empty_in: inLines !== null
      && (inLines ? parseFindings(inLines) : []).length === 0
      && (inLines || []).some(l => IN_TRIED_ITEM_RE.test(l)),
...
module.exports = { parse, PROPOSALS, OOC_GLOSS_NGUOI };
