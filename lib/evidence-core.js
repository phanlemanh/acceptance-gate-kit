 ...
 *
 * Pure-ish: no stdin/stdout/exit, but TWO functions read sibling files when a
 * fileDir is given — `evaluateEvidence` reads contract.md (for the T3 rule),
 * and `evaluateContractWrite` reads gap-probe.md + decisions.jsonl (for the
 * gap-probe guard). Everything else is a function of the payload +
 * caller-supplied config text.
 */
...
function evaluateContractWrite(newPayload, oldPayload, opts) {
  ...
  // Gap-probe guard — xét đúng khoảnh khắc contract TIẾN QUA Cổng 1, không phải
  // mọi lần contract đang ở trạng thái đã duyệt. Hai lỗi mà một điều kiện chữa:
  //  - bám `status === 'approved'` thôi thì mỗi lần sửa một dòng của contract đã
  //    duyệt lại bị đánh giá lại (chặn oan suốt giai đoạn implement, và thông
  //    điệp nói "không được duyệt" trong khi chẳng có gì đang được duyệt);
  //  - chỉ xét `approved` thì đường nhảy cóc draft → implemented/verified/
  //    signed-off đi vòng qua guard hoàn toàn — đúng đường mà luật Cổng 1 ngay
  //    bên trên tồn tại để chặn.
  // T1 (hoặc contract không khai risk_tier) không có nghi thức gap-probe nào để
  // thiếu, nên im lặng tuyệt đối.
  const ADVANCED = /^(approved|implemented|verified|signed-off)$/;
  const tier = (frontmatterField(newPayload, 'risk_tier') || '').toUpperCase();
  if (ADVANCED.test(status) && !ADVANCED.test(oldStatus || '') && (tier === 'T2' || tier === 'T3')) {
    ...
    if (state === 'probe-failed') {
      ...
      if (descope) {
        ...
        // Marker `gap_probe_expected` do BÊN SINH CONTRACT ghi — bước S1 của
        // feature-loop (luồng DUY NHẤT có gap-probe). Luồng `/acceptance` thuần
        // không có bước đó nên contract của nó KHÔNG mang marker và chỉ bị NOTE
        // — đúng như README/QUICKSTART vẫn hứa. Vắng marker = nhắc thì được,
        // chặn thì không: luật mới không hồi tố lên artifact cũ (chuẩn F).
        // KHÔNG ghi số version vào đây — bản trước trỏ "≥1.19" trong khi
        // feature-loop mới 1.16.1, khiến nhánh chặn là code chết mà test vẫn
        // xanh vì fixture tự tay viết marker.
        //
        // Đọc marker từ CẢ oldPayload: nếu chỉ đọc bản sau-ghi thì chính lần
        // ghi tiến cổng có thể xoá dòng marker để tự tháo răng, không dấu vết —
        // trong khi mọi lối thoát khác của kit đều để lại vết (gate1_skipped bị
        // pre-merge NOTE; bỏ gap-probe phải có entry ledger).
        const hasMarker = p => /^(true|yes|1)$/i.test(frontmatterField(p, 'gap_probe_expected') || '');
        const markerNow = hasMarker(newPayload);
        const markerBefore = oldPayload != null && hasMarker(oldPayload);
        if (markerBefore && !markerNow) {
          notes.push('Contract TỪNG KHAI `gap_probe_expected: true` nhưng lần ghi này đã gỡ marker đi — vẫn áp luật gap-probe. Muốn bỏ nghi thức thì ghi entry descope trong decisions.jsonl, đừng gỡ marker.');
        }
        const expected = markerNow || markerBefore;
