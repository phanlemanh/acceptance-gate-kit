  ...
  if (ADVANCED.test(status) && !ADVANCED.test(oldStatus || '') && (tier === 'T2' || tier === 'T3')) {
    ...
    if (state === 'probe-failed') {
      ...
      if (descope) {
        ...
        // Marker `gap_probe_expected` do BÊN SINH CONTRACT ghi —
        // references/contract-template.md (frontmatter) và bước S1 của
        // feature-loop. Vắng marker = workspace sinh trước nghi thức này →
        // nhắc thì được, chặn thì không: một luật mới không được hồi tố lên
        // artifact cũ (chuẩn F). KHÔNG ghi số version vào đây — bản trước từng
        // trỏ "≥1.19" trong khi feature-loop mới ở 1.16.1, khiến nhánh chặn là
        // code chết mà test vẫn xanh vì fixture tự tay viết marker.
