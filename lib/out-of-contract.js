...
function parseFindings(lines) {
  ...
  for (const raw of lines) {
    ...
    if (title) {
      if (cur) out.push(cur);
      cur = { title: title[1], file: '', severity: '', proposal: '', proposal_raw: '', plain: '', harm: '' };
      continue;
    }
    if (!cur) continue;
    const kv = /^\s+(file|severity|Đề xuất|proposal|Người dùng thấy gì|Tác hại)\s*:\s*(.+?)\s*$/.exec(raw);
    if (!kv) continue;
    const k = kv[1];
    ...
    // Tác hại (K7): behavior = lỗi ở vật giao · measure = lỗi độ chặt phép đo. Giá trị lạ
    // hoặc dấu «-» của S4 → rỗng; file đời cũ không có dòng này thì mọi mục rỗng (đọc-cũ).
    else if (k === 'Tác hại') cur.harm = (v === 'behavior' || v === 'measure') ? v : '';
