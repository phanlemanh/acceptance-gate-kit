...
// KHÔNG dùng \b sau chữ tiếng Việt: «có» kết bằng «ó», không phải ký tự-từ ASCII
// nên \b không bao giờ khớp ở đó (lớp ranh-giới-từ-với-dấu, đã dẫm ở grep GNU).
// Neo bằng đầu dòng + lớp ký tự viết hoa/thường là đủ và đọc được.
const OOC_NOISE_RE = /^(Các lỗi dưới đây là thật|\(?\s*[Kk]hông có|\(?\s*n-a(?![\w-]))/;
...
function parse(text) {
  ...
  // Đếm chữ THẬT của mục: bỏ dòng rỗng, câu mở đầu chuẩn, và lời khai rỗng.
  const meat = outLines
    ? outLines.map(l => l.trim()).filter(l => l && !OOC_NOISE_RE.test(l)).join('').length
    : 0;
  return {
    ...
    suspect_empty: outLines !== null && findings.length === 0 && meat >= SUSPECT_MIN_CHARS,
  };
}
