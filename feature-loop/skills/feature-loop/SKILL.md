...
**Nghi thức re-pin (1 lượt lane, N chữ ký — dùng cho MỌI sự kiện re-pin machine-only):** một sự kiện re-pin = dispatch **1 agent tươi** chạy làn bằng MÁY tại HEAD: `node "$WORKFLOWS_DIR/../scripts/repin-lane.mjs" --root . --slug <a> --slug <b> … --reason "<lý do 1 dòng>" --write` (kit tự host thêm `--ag-root .`; env như khi chạy S4 — `CLAUDE_PLUGIN_ROOT`…). Script chạy `feature_loop.suite_keys` **VÀ mọi eval `test`/`script` trong `evals.yaml` của TỪNG slug TRỪ ô tự khai `status: not-run`** (lệnh trùng chỉ chạy một lần; từ 2.12.0 ô khai không-chạy KHÔNG bị thi hành — bên viết và bên đọc rút danh sách từ MỘT hàm dùng chung, và pin phải nói ra ô bị loại, xem khuôn dưới), mint `run_id` mới, `sha = git rev-parse HEAD` (cây bẩn ngoài `_acceptance/` → từ chối), rồi: **bất kỳ suite hay eval exit ≠ 0 → DỪNG: KHÔNG append dòng repin, KHÔNG sửa evidence (script exit 1, không ghi gì) — khắc phục nguyên nhân rồi chạy lane MỚI (run_id mới), không ký mù** — **và cũng DỪNG như vậy khi executor chạm cây `_acceptance/<slug>/` của BẤT KỲ hồ sơ `signed-off`/`machine-cleared` nào** (làn chụp băm + mtime trước suite đầu, sau eval cuối, in đường từng tệp bị chạm; luật tạo phẩm ra `.acceptance-runs/<slug>/`: `eval-executors.md` mục «Where a run writes its artifacts») (từ 07/09/2026: làn chỉ chứng suite từng cho một hồ sơ mất tiền đề đi thẳng nhánh chính với CI xanh — eval của chính hồ sơ mới là thứ pin hứa). Làn xanh → script tự làm với TỪNG slug: (1) append 1 dòng đúng khuôn REPIN-TEMPLATE dưới đây vào run-log.jsonl CỦA TỪNG slug (`_acceptance/<slug>/run-log.jsonl`) — cùng một `run_id` cho cả sự kiện, `evals_exit` = {id eval máy: exit}; (2) evidence-report: frontmatter `verified_commit` → `sha` (bất biến `verified_commit` == `sha` của dòng repin) + append section `### Re-pin lần <N>` đúng khuôn, cite `run_id` nguyên văn — KHÔNG đụng chữ ký người (human_signoff/human_override); (3) tự kiểm bằng `recheck-evidence.cjs` — đỏ thì exit 1 trước khi ai commit. Agent KHÔNG viết tay dòng repin/section: bên đọc đòi đúng vật script sinh. Xong hết N slug, báo user 1 dòng: "re-pin N slug bằng 1 lượt lane (run_id <id>)". Lưới máy đối chiếu (pre-merge + recheck): run_id cite mà không có dòng repin khớp trong run-log slug đó / `sha` dòng repin ≠ `verified_commit` / `suites_exit` có phần tử ≠ 0 / run-log vắng file / **làn chống lưng `verified_commit` thiếu `evals_exit`, thiếu eval máy nào của `evals.yaml` (id tự khai `status: not-run` KHÔNG tính là thiếu — 2.12.0), có eval ≠ 0, mang mã ≠ 0 cho chính một id đã khai không-chạy, hoặc hồ sơ không có `evals.yaml`** — đều là VIOLATION; và luật HAI VẾ của lời khai không-chạy (2.12.0, cùng khuôn ADR 0016): ô khai `status: not-run` mà báo cáo ĐÃ KÝ vẫn có khối `- eval:` mang mã thoát cho chính nó là hai vế mâu thuẫn (làn dừng exit 2 TRƯỚC khi ghi byte nào; bên đọc ghi VIOLATION), và không đọc được báo cáo đã ký để đối chiếu vế hai cũng là VIOLATION (fail-closed — «thêm một dòng khai» không được thành đường né đo); KHÔNG có mốc ngày, KHÔNG sử liệu (owner 08/09/2026): dòng repin thiếu `evals_exit` là VIOLATION dù ghi khi nào — hồ sơ ghim bằng làn suite-only đời cũ là nợ thật, phải ghim lại bằng làn eval (thường ở chiến dịch phát hành); section Re-pin cũ không cite run_id theo khuôn thì không luật nào áp (grandfather). Giới hạn khai: eval `ui-check`/`judgment` không chạy được trong làn máy — re-pin không chứng lại chúng, và từ 2.18 làn NÓI RA điều đó thay vì im: `evals_not_machine` liệt mọi id ngoài làn máy, `evals_not_machine_touched` liệt những id mà diff từ pin cũ tới HEAD chạm `paths` của chính chúng, và section Re-pin nêu thêm `AC không có chốt máy` (AC mà MỌI eval phủ nó đều ngoài làn máy hoặc tự khai không-chạy). Làn KHÔNG chặn ở đây (owner quyết 20/09) — hồ sơ mang `evals_not_machine_touched` là hồ sơ phải đi vòng S4 delta (carry P1 theo `paths`), không đi re-pin; ngưỡng mở vòng chặn đếm bằng lệnh ở GUIDE §7.1.
...
(`ui-check`/`judgment`) theo thứ tự bản khai — **KIỂU thắng TRẠNG THÁI**: một ô
ngoài làn máy khai `status: not-run` chỉ vào khoá này, KHÔNG vào `evals_not_run`.
`evals_not_machine_touched` là tập con của nó, gồm các id mà
`git diff <pin cũ> HEAD` chạm một glob trong `paths` của CHÍNH eval đó — pin nói
ra rằng vật một ô ngoài làn máy đo đã đổi mà lượt ghim này không chứng lại.
Hồ sơ không có ô nào thì khoá VẮNG HẲN. Ba tập — khoá của `evals_exit`,
`evals_not_run`, `evals_not_machine` — rời nhau và hợp lại BẰNG tập id của
`evals.yaml`.

Dòng `sha:` mang thêm ba hậu tố TUỲ CHỌN (script tự nối, KHÔNG viết tay). Hai
hậu tố đầu khi hồ sơ có eval khai `expected_exit` (2.11.0, ADR 0016): ` ·
đạt-có-giới-hạn: <E>=<mã>` cho mỗi eval đạt ĐÚNG một mã khác 0 đã khai, và ` ·
...
0. **Đường nền hạ tầng — TRƯỚC khi viết tệp nào của vòng, trên cây chưa đụng.** Chạy NỀN (tiến trình nền, máy được báo khi xong — chân suite ở kho lớn chạy hàng chục phút, không lời gọi công cụ nào được ôm trọn nó): `node "$WORKFLOWS_DIR/../scripts/duong-nen.mjs" --root . --slug <slug>`; brainstorm đi song song, không chờ. Mã 0 → nền xanh. Mã 1 → `_acceptance/<slug>/duong-nen.md` gọi tên từng thứ đỏ: máy TỰ gỡ thứ gỡ được trong phạm vi repo (sửa lệnh executor sai đường, chạy lệnh cài phụ thuộc repo đã khai) rồi chạy lại; thứ chỉ người gỡ được (cài phần mềm hệ thống, quyền, khoá) gom MỘT lần vào lời mời Cổng Phạm vi — không hỏi rải rác. Đỏ ở đây theo định nghĩa KHÔNG phải lỗi của vòng. Mã 2 → không chạy được (không phải kho git, thiếu `_acceptance/config.yaml` hoặc `feature_loop.suite_keys`): trình nguyên văn thông điệp, không tự dựng tệp thay. Thẻ Cổng Phạm vi đọc tệp này thành khối «Nền hạ tầng».
1. Invoke `superpowers:brainstorming` — hỏi đáp làm rõ như thường lệ. **Mỗi câu XIN DUYỆT thiết kế** của brainstorm (câu «dừng chờ đồng ý» có hình dạng biết trước; có thể nhiều câu, không biết trước câu nào cuối) in kèm khối `GOAL-TEMPLATE` (thay `<slug>`) với một dòng: «nếu đồng ý, dán luôn dòng này cùng câu trả lời» — đây là điểm vũ trang `/goal` ĐẦU TIÊN, lượt người ngay trước đoạn máy S1-đuôi → Cổng 1 (làn V T2 không chạm UI: cũng là lượt người cuối của cả vòng). Brainstorm không hỏi gì thì không có lượt để vũ trang — ca chưa phủ, đo ở ba dòng số.
...
4. Kết thúc brainstorm — 🎨 feature chạm UI: TRƯỚC khi sinh 3 artifact, đọc key `feature_loop.ui_standards_skill` trong `_acceptance/config.yaml` (giá trị = tên skill chuẩn-plugin/DS của repo tiêu thụ, vd `create-<org>-plugin`); key CÓ → BẮT BUỘC invoke skill đó ngay (đối trọng chuẩn nội đặt lên cùng bàn cân với vật liệu ngoài — retro V1 quy luật meta 1: model neo vào khối context mạnh nhất); key VẮNG → ghi chú đúng 1 dòng vào gói Gate 1 ("repo chưa khai `feature_loop.ui_standards_skill` — artifact UI không có đối trọng chuẩn nội"), KHÔNG chặn. Kế đó, VẪN TRƯỚC khi sinh 3 artifact: điền section `## Đặc tả UX` vào design-doc theo khuôn `ux-spec-template.md` của plugin acceptance-gate (resolve bằng `AGX=$(node "$WORKFLOWS_DIR/../scripts/resolve-plugin.mjs" --plugin acceptance-gate --require skills/acceptance/references/ux-spec-template.md)` rồi đọc `$AGX/skills/acceptance/references/ux-spec-template.md` — KHÔNG hardcode path cache; resolver exit khác 0 nghĩa là bản acceptance-gate đang cài CHƯA có khuôn → ghi đúng 1 dòng vào gói Cổng 1 («repo đang chạy bản acceptance-gate chưa có khuôn đặc tả UX») rồi ĐI TIẾP, KHÔNG dừng vòng), GIỮ NGUYÊN các marker; máy điền MỘT lượt, không gọi owner. Mục «Khuôn IA đã chọn + căn cứ»: máy KHÔNG tự chắc khuôn (≥2 khuôn khả dĩ) → tra mẫu theo thang 2 nấc của khuôn ((i) phiên có công cụ tra mẫu thị trường, vd MCP Mobbin → tra luồng cùng loại, ghi vết 1 dòng đã xem gì + rút gì; (ii) không có → chọn từ danh sách khuôn IA có tên, ghi lý do) — không phụ thuộc công cụ nào, luồng hiển nhiên không tra. Contract ghi `design_doc: <path design-doc>` trong frontmatter để người duyệt mở thẳng; GIỮ NGUYÊN các marker của khuôn (mỏ neo cho phép đo khớp vòng sẽ làm ở việc riêng — `docs/plans/2026-08-24-hat-giong-khop-vong-dac-ta-ux.md`). Vòng này KHÔNG có phép đo máy nào cho đặc tả UX: người duyệt soi nó tại Cổng 1 cùng ba artifact. Feature KHÔNG chạm UI → bỏ bằng entry `descope` AUTO-DRAFT với decision bắt đầu đúng chuỗi `"bỏ đặc-tả-UX — <lý do 1 dòng>"` + impact "cấu trúc không khai trước — owner thấy nó lần đầu trong code". Rồi sinh CÙNG LÚC từ một ngữ cảnh:
   ...
   - `_acceptance/<slug>/evals.yaml` — map mỗi AC ≥1 eval; executor ưu tiên test > script > ui-check > judgment; cmd PHẢI là `config:` ref (vd `config:executors.test.api`), KHÔNG hardcode lệnh. Kế hoạch đo theo khuôn `MEASURE-BIRTH-CLAUSE` (khối ở S3): `expected` của mỗi eval máy phải khai được CHIỀU ĐỎ — cặp hai-chiều cùng fixture + thông điệp ghim. Mỗi eval máy/ui NÊN khai `paths: [<glob tương đối repo>]` = các file eval này THẬT SỰ kiểm — dùng cho carry-forward round delta (P1, Đợt 5); thiếu `paths` → eval LUÔN chạy lại (mặc định an toàn); criterion tag `(cross-layer)` → thêm ≥1 eval `layer: backend-effect` theo Pairing mechanics của eval-executors.md (ui-check một mình không bao giờ đủ cho criterion xuyên lớp); surface mặt người nhìn (`ui`, alias `web`/`web-ui`, không tính `mobile`) → ≥1 eval `ui-check` khai `layer: ui-observed` THEO HỢP ĐỒNG — test/script cho UI không trả nghĩa vụ này (luật gương của cross-layer, mục 2c của acceptance SKILL); bỏ → entry `descope` AUTO-DRAFT với decision bắt đầu đúng chuỗi `"bỏ ui-observed — <lý do 1 dòng>"` + impact "Cổng Bằng chứng không có frame — người ký đọc tên ca máy thay vì nhìn"
   - Cuối S1: append entry ledger cho approach/descope thỏa rule đáng-log (xem "Sổ quyết định").
5. **KHÔNG vào Gate 1 khi chưa đủ 3 artifact — và (CT-S) contract THIẾU section `## Coverage` (dù chỉ 1 dòng skip) cũng tính là chưa đủ.** (HARD-GATE của brainstorming và Gate 1A/1B của kit gộp thành MỘT Gate 1; dòng `/goal` in kèm câu xin duyệt ở S1#1 KHÔNG phải một cổng — nó đi cùng câu trả lời, không thêm lượt.) Ngược lại, đủ điều kiện rồi (3 artifact + Coverage + gap-probe đã định đoạt) → render thẻ Gate 1 NGAY trong cùng lượt, không chèn câu hỏi trung gian nào trước đó.
...
(Khối trên là bản runtime — bản người-đọc kèm giải thích nằm ở GUIDE mục /goal, bản thứ ba là hằng trong `scripts/gate-card.js` để thẻ Cổng 1 in dòng goal; ba bản được test P85 giữ khớp (so sau strip, 6 dòng). Dán thành 1 dòng khi dùng; xuống dòng chỉ để dễ đọc.)
...
1. Chuẩn bị args — **MỘT lệnh, máy sinh trọn tệp** (script chạy NGOÀI workflow vì workflow không có filesystem; bản soạn-tay-theo-văn-xuôi là lớp lỗi có đo — hồ sơ cham-dung-cay-dung-cho-dung, điều tra 29/08):

   `node "$WORKFLOWS_DIR/../scripts/s4-args.mjs" --slug <slug> --root <repo-root> --out <ws>/s4-args.json`

   Round ≥ 2 PHẢI khai đường carry tường minh («quên» = script exit 2, không phải trạng thái lặng): thêm `--carry-anchor <sha>` (round delta: `verified_commit` cũ từ Staleness guard; round fix: `sha` dòng run-log round trước) để script tính carry, hoặc `--no-carry` khi cố ý full re-run.
   Script tự làm trọn các việc từng phải soạn tay, fail-CLOSED từng phần (thiếu/ref hỏng → exit ≠ 0 ghim tên, KHÔNG sinh tệp, không đoán): resolve `config:` ref và GIỮ `ref` gốc trên từng eval (hook L2 đối chiếu `verifier:` bằng ref, không nhận lệnh resolved) · `suiteCommands` từ `feature_loop.suite_keys` · `inputs` judgment tính từ GỐC KHO (cùng gốc với `paths`) → abs path, input vắng trên đĩa hoặc là thư mục → exit 2 gọi tên file, không sinh tệp; riêng `_acceptance/<slug>/evidence/**` của chính hồ sơ được phép chưa có (ui-check sinh trong lúc chấm) — giải kèm một dòng khai, vắng lúc chấm thì hội đồng UNCERTAIN · resolve plugin acceptance-gate qua `resolve-plugin.mjs` (KHÔNG glob cache — `ls` sắp theo CHỮ nên "bản mới nhất" chọn nhầm bản cũ) với `--require skills/acceptance/references/judge-personas.md --require skills/acceptance/references/evidence-report-template.md --require skills/acceptance/references/tool-kill-rule.md`, rồi đọc **`toolKillRule` NGUYÊN VĂN** từ file nguồn (nguồn DUY NHẤT của luật «lệnh bị công cụ ngắt ≠ lệnh fail»; workflow tự rút khối marker, thiếu → BLOCKED `(args)` có tên) · `riskTier` từ contract frontmatter · `diffBase` = merge-base với nhánh chính (detect remote → fallback main/master/develop/trunk) · `contractPath` cho scope-triage · `invokedAt`/`invokedSha` · `round` đếm từ `## Iterations` (đếm không được → exit ≠ 0, chống mint trùng run_id) · carry P1 (tự gọi `carry-plan.mjs` — đủ ma trận paths-vs-delta + atomic-pair `(cross-layer)`; suite LUÔN chạy lại) · P2 (`evalsHash`, baseline-once theo dòng `kind:"baseline"`) · P3 (`inputsHash` per judgment, panel memo theo dòng `kind:"panel"`) · `models` từ `feature_loop.models` nếu có. Chuỗi lệnh trong tệp giữ SẠCH — ghim chỗ đứng là việc của từng LANE trong workflow (verifier `cd <repoRoot> && …`, baseline `cd "$WT" && …`), KHÔNG nướng `cd` vào args.
   - Repo tự-host kit (kit là nguồn plugin) → thêm `--ag-root <repo-root>`. Nhánh chính không detect được → script exit có tên, truyền `--diff-base <ref>` tường minh; KHÔNG để `git merge-base` fail mờ.
   - Repo có skill review invariant riêng (vd `.claude/skills/<review-skill>/SKILL.md`) → sau khi đọc tệp args, thêm field `reviewSkillPath` (abs path) vào object trước khi invoke; không có → bỏ qua, workflow tự review theo conventions.
   - `config.yaml` thiếu `feature_loop.suite_keys` → script exit 2 có tên. Hỏi user MỘT lần: liệt kê key đang có trong `executors.*`, user chọn lệnh chạy mỗi round → GHI **bằng script splice chung, KHÔNG tự edit tay**: `node <acceptance-gate-plugin-root>/scripts/config-patch.mjs --config _acceptance/config.yaml --key feature_loop.suite_keys --value "[<key1>, <key2>]" --write`, rồi chạy lại s4-args (lần sau không hỏi lại). KHÔNG đoán theo Node convention, KHÔNG tự lấy toàn bộ `executors.*` — itest của feature KHÁC có thể flaky đốt round.
   - Round fix có carry → danh sách eval carried phải nằm RÕ trong báo cáo user + gói Cổng 2, kèm lý do từng eval (luật minh bạch ở bước "Mọi verdict").
   <!-- <<<S4-ARGS-CLAUSE -->
   **Script sinh-args lỗi (exit ≠ 0) → DỪNG, trình người NGUYÊN VĂN thông điệp
   của script. TUYỆT ĐỐI KHÔNG rơi về soạn args bằng tay theo trí nhớ hay văn
   xuôi** — args soạn tay chính là lớp lỗi vòng này đóng (agent giải nhầm chỗ
   ...
   **Tệp args có hạn dùng theo cây:** tệp do `s4-args.mjs` sinh mang
   ...
   `generated_sha` — HEAD khác nghĩa là cây đã đổi sau khi sinh: SINH LẠI tệp
   args rồi mới invoke, KHÔNG dùng tệp cũ (args trỏ cây cũ là bằng chứng neo
   sai cây — lớp lỗi verify-trước-commit).
   <!-- S4-ARGS-FRESHNESS>>> -->
2. Invoke: `Workflow({ scriptPath: '<WORKFLOWS_DIR>/acceptance-verify.js', args: { slug, round, riskTier, evals, suiteCommands, diffBase, repoRoot, personasPath, templatePath, toolKillRule, contractPath, reviewSkillPath?, invokedAt, invokedSha, carriedEvals?, carriedPanels?, runBaseline?, carriedAnalyst?, evalsHash?, vungVat?, ngoaiVatGlobs?, carriedFindings? } })` (debug fan-out không tốn agent: thêm `dryRun: true` → trả về distinctCommands/judgePanels + carried plan + `finders` (lane THẬT SỰ chạy lượt này) + `vungVat`, không chạy gì).
