...
4. Kết thúc brainstorm, sinh CÙNG LÚC từ một ngữ cảnh:
   ...
   - `_acceptance/<slug>/contract.md` — theo template plugin acceptance-gate (frontmatter schema_version/feature/slug/risk_tier/surfaces/status: draft, KÈM `gap_probe_expected: true` — marker BẮT BUỘC ở luồng này vì chỉ feature-loop mới chạy S1#7; nó là thứ cho hook quyền chặn T3 thiếu gap-probe, vắng marker hook chỉ NOTE; 5-15 AC Given/When/Then, tag `(judgment)` cho business-judgment; section `## Coverage` từ bước CT-S; Out of scope ≥2 bullet)
...
1. Chuẩn bị args (main loop đọc file, script không có filesystem):
   ...
   - Resolve plugin acceptance-gate (KHÔNG hardcode version, KHÔNG glob cache): `AG=$(node "$WORKFLOWS_DIR/../scripts/resolve-plugin.mjs" --plugin acceptance-gate --require skills/acceptance/references/judge-personas.md --require skills/acceptance/references/evidence-report-template.md)` → `personasPath` = `$AG/skills/acceptance/references/judge-personas.md`, `templatePath` = `$AG/skills/acceptance/references/evidence-report-template.md`. Exit khác 0 → DỪNG, in nguyên văn thông điệp của resolver (đã kèm lệnh cài) — preflight S0 lẽ ra đã bắt. **Vì sao không glob:** `ls` sắp theo thứ tự CHỮ, nên với cache có 1.9.2/1.11.2/1.18.0/1.20.1 thì "bản mới nhất" ra 1.9.2 — panel judge và template evidence sẽ chạy bằng luật của bản cũ mà report vẫn xanh.
   ...
   - `models` (optional) = đọc block `feature_loop.models` trong config.yaml nếu có (mỗi dòng con `<role>: <model>`; role: machine/ui/judge/finder/refute/baseline/provenance/scribe/synthesize/critic — critic dùng ở S1#7 gap-probe, KHÔNG dùng trong S4 — map vẫn truyền nguyên, sanitize của acceptance-verify.js tự bỏ role lạ; giá trị `session` = kế thừa model phiên chính) → truyền nguyên map. Không có block → bỏ qua field, script dùng default. KHÔNG tự bịa model theo cảm tính — bảng default trong script là quyết định đã cân nhắc.
