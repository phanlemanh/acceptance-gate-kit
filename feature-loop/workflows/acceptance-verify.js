...
// HAI GIONG, CO CHU DICH: `reason` la tieng may (di vao result.inertFields cho consumer
// va log), `plain` la tieng NGUOI (di vao inertNote -> ## Variance -> the Cong 2). Phep thu:
// xoa moi ten may khoi `plain` thi cau van con noi duoc dieu gi cho nguoi quyet — nen `plain`
// khong duoc chua "P1"/"P3"/"carry-forward"/"3-lens" (case WI5 ghim dieu nay).
// <<<INERT-FIELD-TABLE
const INERT_FIELD_TABLE = [
  { field: 'runs', executor: 'judgment',
    reason: 'panel 3-lens da la co che hap thu nhieu da chon — runs khong duoc doc',
    plain: 'eval hội đồng luôn chạy đúng một lần mỗi góc nhìn, và ba góc nhìn đã là cách hấp thụ sai số' },
...
const inertFieldReport = (evals) => {
  ...
  for (const e of (Array.isArray(evals) ? evals : [])) {
    for (const row of INERT_FIELD_TABLE) {
      ...
      out.push({ evalId: e.id, field: row.field, value: e[row.field], executor: e.executor, reason: row.reason, plain: row.plain })
    }
  }
  return out
}
...
// Chuan hoa path o BIEN truoc khi so glob: reviewer agent duoc nhac "trong repo
// <abs path>" nen tra path tuyet doi la hop le theo schema, va khi do MOI finding
// rot khoi MOI glob -> co cum bia ra o moi round. Chuan hoa phong thu la du; khong
// dua vao viec agent nghe loi.
...
// scripts/gate-card.js (hai file khong import duoc nhau — script nay chay trong sandbox
// khong co module). Doi cum nay = PHAI doi ca ben do; case WI6 trong tests/workflows la
// moi noi duy nhat giu hai ben khop.
