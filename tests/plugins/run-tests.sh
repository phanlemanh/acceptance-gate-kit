...
# --- design-pass cases (P58-P67) begin ---
...
run "P58 design-pass frontmatter + NOT-for + open invocation (E1)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P59 design-pass preflight: keys + {slug} template + DUNG + standalone slug (E2)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P60 design-pass 2 nguon luat + thang DS + shadcn default (E3)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P61 design-pass thang vat lieu + khai material + cam tu dung (E4)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P62 design-pass 4 luat cung thanh van (E5)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P63 design-pass vong lap owner-phan-ung + cam tu cham (E6)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P64 design-pass ket phien: duong capture rieng + cam CT2 + provenance + states (E7)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P65 design-pass khuon marker: round-trip + than tro toi marker (E8)" \
  python3 - "$ROOT" <<'PY'
import re, sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P66 design-pass engine-clean + mot mat phang (E9)" \
  python3 - "$ROOT" <<'PY'
import sys
from pathlib import Path
root = Path(sys.argv[1])
...
run "P67 design-pass smoke DUONG ban mirror (E11)" \
  python3 - "$ROOT" <<'PY'
import sys
from pathlib import Path
root = Path(sys.argv[1])
