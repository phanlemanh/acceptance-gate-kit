...
Fetch with whatever the harness provides — in Claude Code, `WebFetch` for the HTML/CSS, or the Browser pane when you need a rendered screenshot for color sampling; in other harnesses, any HTML fetch works. If the site has multiple pages worth sampling (landing + blog + product), fetch 2–3 and merge the palette signals.
...
Show the user the token table that will be written to `<repo>/docs/reference/diagram-skin.md` (schema: `DIAGRAM-SKIN-TEMPLATE` in `style-guide.md` — marker + eight roles + two font stacks, nothing else). `style-guide.md` itself is NOT edited.
...
Also regenerate the dark variant via the inversion rule in `style-guide.md` (ink-rgb and paper-rgb swap at the same opacities, recomputed for the new values).

---

## Step 5 — apply

Write `<repo>/docs/reference/diagram-skin.md` from the `DIAGRAM-SKIN-TEMPLATE` with the new tokens and **`skin: custom` on its first line** — that file, in the repo, is what tells the first-run gate the decision was made, and it travels with the repo to every teammate. Do NOT write into `style-guide.md` or anywhere inside the skill (shared package). Then rebuild one example to verify the new skin reads cleanly.
...
Or the first-run gate (SKILL.md §0) identified this skill as the brand source and the user approved.
...
Or the first-run gate (SKILL.md §0) identified this folder as the brand source and the user approved.
