...
- `duyệt[: <tên> [<ngày>]]` → chính là câu YES tường minh của bước 5:
  ...
  **CHỌN** giá trị ở nấc cao nhất còn tên: `--as "<tên>"`
  ...
  chạm. Suy xong: hai nguồn độc lập KHỚP TUYỆT ĐỐI (`signoff.approvers` đúng
  một tên VÀ bằng `git config user.name`) → ghi thẳng rồi hiển thị lại «với
  danh tính: <tên> <ngày> (từ <nguồn suy>)», không chờ; mọi ca khác thì HIỂN
  THỊ LẠI «với danh tính: <tên> <ngày> (từ <nguồn suy>) — Enter xác nhận»
  TRƯỚC khi ghi.
