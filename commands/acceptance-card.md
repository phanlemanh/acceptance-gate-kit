...
4. **Nạp luật TRƯỚC khi viết:** đọc `${CLAUDE_PLUGIN_ROOT}/skills/acceptance/references/human-facing-language.md`
   ...
   **Translate** the extract into PLAIN PRODUCT LANGUAGE for this repo's persona
   ...
   - Gate 2 khối "Ngoài hợp đồng": **KHÔNG dịch, KHÔNG có key overlay cho khối
     ...
     (a) **ghi Known limits** — chấp nhận, ghi vào phần hạn chế đã biết, ship bản này; (b) **mở hợp đồng mới** — <!-- <<<OOC-LOI-B -->«ghi hạt giống có Gốc, chờ kho gọi tên — không mở ô»: ghi HẠT GIỐNG `docs/plans/<ngày>-hat-giong-<slug>.md` có dòng `Gốc:` trỏ hồ sơ vòng này + phát hiện; KHÔNG tạo `_acceptance/<slug>/` (luật 18/09)<!-- OOC-LOI-B>>> -->; (c) **nâng phạm vi sửa ngay** — bổ sung tiêu chí vào hợp đồng hiện tại rồi duyệt lại Cổng 1.
