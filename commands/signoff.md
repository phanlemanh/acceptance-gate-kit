...
- «Ngoài-<số>: ghi Known limits / mở hợp đồng mới / nâng phạm vi sửa ngay»
  ...
  <!-- <<<OOC-LOI-B -->«ghi hạt giống có Gốc, chờ kho gọi tên — không mở ô»: ghi HẠT GIỐNG `docs/plans/<ngày>-hat-giong-<slug>.md` có dòng `Gốc:` trỏ hồ sơ vòng này + phát hiện; KHÔNG tạo `_acceptance/<slug>/` (luật 18/09)<!-- OOC-LOI-B>>> -->
