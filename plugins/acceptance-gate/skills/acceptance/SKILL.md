...
`.codex/agents` policy is installed, select `acceptance_ui_verifier`,
`acceptance_judge`, `acceptance_reviewer`, and `acceptance_refuter` by name only
...
4. Coverage check — three rules:
   ...
   should-NOT-fire case (W1), out-of-scope items with zero negative evals (W3), and `(cross-layer)` criteria with no `layer: backend-effect` eval (W4).
...
2. The subagent executes per executor type:
   ...
   - `ui-check`: select `acceptance_ui_verifier` when available, then start dev server per `config:dev_server.start`; drive via the
   ...
   - `judgment`: dispatch `acceptance_judge` per `references/judge-personas.md`
     ...
     itself inline.
   - **Review:** run two `acceptance_reviewer` passes for conventions/invariants
     and bugs/silent failures. Dispatch one `acceptance_refuter` for each
