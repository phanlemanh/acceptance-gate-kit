...
Đọc message — nó nói thiếu gì. 99% là một trong các trường hợp:
...
- **`verified_commit` không phải SHA thật** → điền đúng output `git rev-parse HEAD` lúc verify (đây là mỏ neo chống "code đổi sau verify vẫn xài evidence cũ")
- **`run_id` không có trong `run-log.jsonl`** → run_id phải chép từ lần chạy verifier THẬT (máy ghi log lúc chạy); tự bịa id là bị đối chiếu chặn — chạy verify lại, đừng sửa tay
- **Contract nhảy cóc Cổng 1** (`status: approved/signed-off` hoặc draft → implemented mà `approved_by` rỗng) → duyệt Cổng 1 cho đúng, hoặc user chủ động skip thì ghi `gate1_skipped: true`
...
- **Đo hiệu quả?** `/acceptance-report` đếm tần suất sự-kiện-cần-người từ git
  (2.0.0). Không ai hỏi số phút ở cổng nữa — `time_human_minutes` là tuỳ chọn,
  số cũ chỉ được trình dưới nhãn "tự khai — không đáng tin".
