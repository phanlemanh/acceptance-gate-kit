...
flowchart TB
  subgraph DEV["🖥️ Máy dev — Claude Code + plugins"]
    ...
    WF["<b>Workflows</b><br/>acceptance-verify.js (S4)<br/>execute-parallel.js (S3)"]
    HOOK["<b>Hook PreToolUse</b><br/>acceptance-evidence-gate.js<br/>chặn NGAY LÚC GHI"]
...
| `design-loop` | Làn design cho web UI: mockup chuẩn → so pixel khi verify (tùy chọn) |
| Hook `acceptance-evidence-gate.js` | Chặn PASS thiếu bằng chứng + contract nhảy cóc Cổng 1, ngay lúc agent ghi file |
...
**Vì sao cần cả hook lẫn CI?** Hook chỉ thấy **agent** ghi file (PreToolUse). Người mở
editor sửa tay evidence thì hook mù — CI re-check chạy đúng bộ luật đó trên file **đã
commit**, cộng thêm các kiểm tra chỉ làm được bằng git history (evidence có bị cũ so
với code không, ai đưa chữ ký vào, PR có né gate không).
...
sequenceDiagram
  ...
  participant W as Workflow acceptance-verify
  ...
  Note over W: JS THUẦN (không LLM): merge kết quả, tính verdict,<br/>chốt run_id từng eval, build từng dòng run-log
...
(Codex: bản port chưa phát hành — hiện kit chỉ chạy trên Claude Code.)
...
1. Trong Claude Code: chạy **`/acceptance-init`** — trả lời lệnh test/smoke của repo,
...
Khác biệt: verify chạy bằng một subagent (không fan-out đa agent như S4), nhưng
**cùng template, cùng hook, cùng CI** — mức bằng chứng không đổi.
...
| Workflow S4 đứt giữa chừng | Crash/cancel | Resume `resumeFromRunId` cùng round; đã sửa code → chạy round mới |
