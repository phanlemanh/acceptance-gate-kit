...
- **S0 — detect.** `node ${plugin}/scripts/design-detect-surface.mjs --slug <slug>` → trả `surface:true` = CT1 bật; CT2 KHÔNG bật ở đây — nó bật khi user chạy `/design-mockup` (feature-loop phía Claude nay route mọi feature chạm UI qua nghi thức S1-D design-pass thay cho lane question cũ; bản Codex của feature-loop vẫn giữ lane question riêng của nó).
