...
- [ ] Verify: `ONLY_BLOCK=P197` PASS; `bash _acceptance/hinh-tai-cong-1/rang.sh` OK; phá thử: file stdout thật, xoá một dòng `DO dung` → rang ĐỎ ghim msg; xoá một dòng `P197-M:` → ĐỎ ghim sàn. Commit.
...
- [ ] Răng: `OUT=$(ONLY_BLOCK="P90 tam" … )`, `OUT197=$(ONLY_BLOCK=P197 …)`, `OUT198=$(ONLY_BLOCK=P198 …)`; ghim mọi dòng AC-8; ghi `OUT197` ra `$TMP/p197.out`; chạy `RANG_STDOUT_FILE=$TMP/p197.out bash _acceptance/hinh-tai-cong-1/rang.sh` → OK; sed bản sao xoá 1 dòng `DO dung` → ĐỎ ghim msg; sed xoá 1 dòng `P197-M:` → ĐỎ ghim sàn; bản sao suite bỏ đột biến nhãn `[3] Vẽ` (sed xoá dòng `go nhan [3] Vẽ`) → P197 ĐỎ ghim `ma tran chua toan phan`; grep rang.sh không có `for M in "`; worktree `8d1e135` (mốc trong răng) + copy rang.sh của hinh-tai-cong-1 → ĐỎ.
