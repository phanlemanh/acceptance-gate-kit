...
- [ ] Ca NT-AC3-lanh/do/im như E3; thêm một mũi hình dạng ca thật crm `dieu-phoi` 21/09 (`508ed3f7`): một commit giữa lượt chạm đồng thời tệp test kho và một tệp vật → dòng `thuoc-lech` gọi đúng tệp test, KHÔNG gọi tệp vật (vật ngoài tập thước — giới hạn khai, hạt giống `docs/plans/2026-09-21-hat-giong-tac-nhan-cham-ghi-vao-cay.md`) (ma trận 4 mũi, mỗi mũi bản sao kho riêng; `assert byte đổi` trước khi tin đỏ; mũi test kho = một tệp `tests/a.test.mjs` tracked). Đọc khoá dòng bằng cách rút khối marker từ `thuoc-vat.mjs` (không gõ literal).
- [ ] Chạy → FAIL.
...
- [ ] Hiện thực trong `gate-card.js` Cổng 2:
  ...
  - `'khoa'` và REJECT → nhánh non-approvable CŨ, không đổi byte — TRỪ ca BLOCKED có mục `'vat'`: vẫn khoá, nhưng khối «Vì sao chưa ký được» in mỗi mục một dòng với nhãn riêng («sai hợp đồng — E11 (AC-x)» · «hệ thống chết — E6 (AC-y)»), không gộp một BLOCKED. Ca NC-AC7-tron: fixture đúng hình dạng crm lượt D (một dòng `vang-mat` cho E6 + một dòng eval E11 exit 1, hai dòng `round-tally` BLOCKED cùng round) → approvable false, HTML có cả hai nhãn trên hai dòng.
