...
description: Codex-native design sub-track for a web-UI feature inside feature-loop-codex. Use when designing or changing a screen, plugin view, mockup, or user-facing surface. Requires Acceptance Gate, Feature Loop Codex, and one-time setup with the design-init skill. Not for headless features.
---
...
the `design-init` skill has wired `executors.design.*` (once per repo). In Codex, the
...
- **S0 — detect.** Run `node scripts/codex-plugin-runner.mjs design-loop design-detect-surface --slug <slug>` → `surface:true` enables CT1. CT2 is enabled only by the lane decision or the `design-mockup` skill.
...
- **S1-D — mockup/reference.** **(CT2 only — skip hoàn toàn khi lane static-only)** Invoke the `design-mockup` skill for `<slug>` and perform the
...
- **Gate 2.** **(CT2)** invoke the `design-evidence` skill for the onion-skin panel (reference↔impl); the human resolves the perceptual AC. **(CT1∧¬CT2)** uses normal screenshot/`observed` evidence. Machine verdict + `status → signed-off` stay unchanged.
- **S5 — ship.** PR as usual. Invoke `design-push-status` only to record that optional Claude cloud sync was skipped or handled outside Codex.
